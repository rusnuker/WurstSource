...
public class TileEntityEndGateway extends TileEntityEndPortal implements ITickable
{
    ...
    public void update() {
        ...
        if (flag2) {
            ...
            final List<Entity> list = this.world.getEntitiesWithinAABB((Class<? extends Entity>)Entity.class, new Box(this.getPos()));
            if (!list.isEmpty()) {
    ...
    private BlockPos findExitPosition() {
        ...
        TileEntityEndGateway.LOG.debug("Best exit position for portal at {} is {}", (Object)this.exitPortal, (Object)blockpos);
        return blockpos.up();
    }
    ...
    private void findExitPortal() {
        ...
        for (int i = 16; getChunk(this.world, vec3d2).getTopFilledSegment() > 0 && i-- > 0; vec3d2 = vec3d2.add(vec3d.scale(-16.0))) {
            TileEntityEndGateway.LOG.debug("Skipping backwards past nonempty chunk at {}", (Object)vec3d2);
        }
        for (int j = 16; getChunk(this.world, vec3d2).getTopFilledSegment() == 0 && j-- > 0; vec3d2 = vec3d2.add(vec3d.scale(16.0))) {
            TileEntityEndGateway.LOG.debug("Skipping forward past empty chunk at {}", (Object)vec3d2);
        }
        TileEntityEndGateway.LOG.debug("Found chunk at {}", (Object)vec3d2);
    ...
    private void createExitPortal(final BlockPos posIn) {
        ...
        if (tileentity instanceof TileEntityEndGateway) {
            ...
            TileEntityEndGateway.LOG.warn("Couldn't save exit portal at {}", (Object)posIn);
        }
    }
