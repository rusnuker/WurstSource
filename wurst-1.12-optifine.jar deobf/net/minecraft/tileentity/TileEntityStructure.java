...
public class TileEntityStructure extends TileEntity
{
    ...
    public void readFromNBT(final NBTTagCompound compound) {
        ...
        catch (final IllegalArgumentException var13) {
            this.mode = Mode.DATA;
        }
    ...
    public void setName(final String nameIn) {
        ...
        for (int length = (illegal_STRUCTURE_CHARACTERS = ChatAllowedCharacters.ILLEGAL_STRUCTURE_CHARACTERS).length, i = 0; i < length; ++i) {
            final char c0 = illegal_STRUCTURE_CHARACTERS[i];
            s = s.replace(c0, '_');
        }
        this.name = s;
    }
    ...
    private List<TileEntityStructure> filterRelatedCornerBlocks(final List<TileEntityStructure> p_184415_1_) {
        final Iterable<TileEntityStructure> iterable = Iterables.filter((Iterable)p_184415_1_, (Predicate)new Predicate<TileEntityStructure>() {
    ...
    private List<TileEntityStructure> getNearbyCornerBlocks(final BlockPos p_184418_1_, final BlockPos p_184418_2_) {
        final List<TileEntityStructure> list = Lists.newArrayList();
    ...
    public boolean save(final boolean p_189712_1_) {
    ...
    public boolean load(final boolean p_189714_1_) {
    ...
    public enum Mode implements IStringSerializable
    {
        ...
        static {
            ...
            for (int length = (values = values()).length, i = 0; i < length; ++i) {
                final Mode tileentitystructure$mode = values[i];
                Mode.MODES[tileentitystructure$mode.getModeId()] = tileentitystructure$mode;
            }
        }
        
        private Mode(final String name, final int ordinal, final String modeNameIn, final int modeIdIn) {
