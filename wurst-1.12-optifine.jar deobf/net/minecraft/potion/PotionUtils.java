...
public class PotionUtils
{
    ...
    public static List<PotionEffect> mergeEffects(final PotionType potionIn, final Collection<PotionEffect> effects) {
        final List<PotionEffect> list = Lists.newArrayList();
    ...
    public static int func_190932_c(final ItemStack p_190932_0_) {
    ...
    public static void addPotionTooltip(final ItemStack itemIn, final List<String> lores, final float durationFactor) {
        ...
        final List<Tuple<String, AttributeModifier>> list2 = Lists.newArrayList();
        if (list.isEmpty()) {
            ...
            for (final PotionEffect potioneffect : list) {
                ...
                if (!map.isEmpty()) {
                    for (final Map.Entry<IAttribute, AttributeModifier> entry : map.entrySet()) {
                        ...
                        list2.add(new Tuple<String, AttributeModifier>(entry.getKey().getAttributeUnlocalizedName(), attributemodifier2));
                    }
                }
                if (potioneffect.getAmplifier() > 0) {
                    s2 = String.valueOf(s2) + " " + I18n.translateToLocal("potion.potency." + potioneffect.getAmplifier()).trim();
                }
                if (potioneffect.getDuration() > 20) {
                    s2 = String.valueOf(s2) + " (" + Potion.getPotionDurationString(potioneffect, durationFactor) + ")";
                }
