...
public class CommandFill extends CommandBase
{
    ...
    public void execute(final MinecraftServer server, final ICommandSender sender, final String[] args) throws CommandException {
        ...
        if (args.length >= 8) {
            iblockstate = CommandBase.func_190794_a(block, args[7]);
        }
        else {
                    ...
                    if (args.length >= 9) {
                        if (!"outline".equals(args[8]) && !"hollow".equals(args[8])) {
                            if ("destroy".equals(args[8])) {
                                ...
                                if (args.length > 10 && !"-1".equals(args[10]) && !"*".equals(args[10]) && !CommandBase.func_190791_b(block2, args[10]).apply((Object)world.getBlockState(blockpos5))) {
                                    continue;
                                }
                            }
                        }
