...
public class CommandClone extends CommandBase
{
    ...
    public void execute(final MinecraftServer server, final ICommandSender sender, final String[] args) throws CommandException {
        ...
        if (args.length >= 10) {
            if ("masked".equals(args[9])) {
                ...
                if (args.length >= 13) {
                    predicate = CommandBase.func_190791_b(block, args[12]);
                }
            }
        }
        final List<StaticCloneData> list = Lists.newArrayList();
        final List<StaticCloneData> list2 = Lists.newArrayList();
        final List<StaticCloneData> list3 = Lists.newArrayList();
        final Deque<BlockPos> deque = Lists.newLinkedList();
        ...
        final List<StaticCloneData> list4 = Lists.newArrayList();
        list4.addAll(list);
        list4.addAll(list2);
        list4.addAll(list3);
        final List<StaticCloneData> list5 = Lists.reverse((List)list4);
    ...
    public List<String> getTabCompletionOptions(final MinecraftServer server, final ICommandSender sender, final String[] args, @Nullable final BlockPos pos) {
        ...
        if (args.length > 6 && args.length <= 9) {
            return CommandBase.getTabCompletionCoordinate(args, 6, pos);
        }
        if (args.length == 10) {
