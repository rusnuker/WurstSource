...
public class CommandSpreadPlayers extends CommandBase
{
    ...
    public void execute(final MinecraftServer server, final ICommandSender sender, final String[] args) throws CommandException {
        ...
        sender.addChatMessage(new TextComponentTranslation("commands.spreadplayers.spreading." + (flag ? "teams" : "players"), new Object[] { list.size(), d4, d0, d2, d3 }));
    ...
    private void spread(final ICommandSender sender, final List<Entity> p_110669_2_, final Position pos, final double spreadDistance, final double maxRange, final World worldIn, final boolean respectTeams) throws CommandException {
        ...
        if (acommandspreadplayers$position.length > 1) {
            sender.addChatMessage(new TextComponentTranslation("commands.spreadplayers.info." + (respectTeams ? "teams" : "players"), new Object[] { String.format("%.2f", d5), i }));
        }
    }
    
    private int getNumberOfTeams(final List<Entity> p_110667_1_) {
        final Set<Team> set = Sets.newHashSet();
    ...
    private double setPlayerPositions(final List<Entity> p_110671_1_, final World worldIn, final Position[] p_110671_3_, final boolean p_110671_4_) {
        double d0 = 0.0;
        int i = 0;
        final Map<Team, Position> map = Maps.newHashMap();
    ...
    public List<String> getTabCompletionOptions(final MinecraftServer server, final ICommandSender sender, final String[] args, @Nullable final BlockPos pos) {
        return (args.length >= 1 && args.length <= 2) ? CommandBase.getTabCompletionCoordinateXZ(args, 0, pos) : Collections.emptyList();
    }
