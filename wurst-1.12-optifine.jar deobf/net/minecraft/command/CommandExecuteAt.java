...
public class CommandExecuteAt extends CommandBase
{
    ...
    public void execute(final MinecraftServer server, final ICommandSender sender, final String[] args) throws CommandException {
        ...
        if ("detect".equals(args[4]) && args.length > 10) {
            ...
            if (!CommandBase.func_190791_b(block, args[9]).apply((Object)iblockstate)) {
                throw new CommandException("commands.execute.failed", new Object[] { "detect", entity.getName() });
            }
            i = 10;
        }
        final String s = CommandBase.buildString(args, i);
        final ICommandSender icommandsender = CommandSenderWrapper.func_193998_a(sender).func_193997_a(entity, new Vec3d(d0, d2, d3)).func_194001_a(server.worldServers[0].getGameRules().getBoolean("commandBlockOutput"));
    ...
    public List<String> getTabCompletionOptions(final MinecraftServer server, final ICommandSender sender, final String[] args, @Nullable final BlockPos pos) {
        ...
        if (args.length > 5 && args.length <= 8 && "detect".equals(args[4])) {
            return CommandBase.getTabCompletionCoordinate(args, 5, pos);
        }
