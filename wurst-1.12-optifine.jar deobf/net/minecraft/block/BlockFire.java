...
public class BlockFire extends Block
{
    ...
    public IBlockState getActualState(final IBlockState state, final IBlockAccess worldIn, final BlockPos pos) {
        return (!worldIn.getBlockState(pos.down()).isFullyOpaque() && !Blocks.FIRE.canCatchFire(worldIn, pos.down())) ? state.withProperty((IProperty<Comparable>)BlockFire.NORTH, this.canCatchFire(worldIn, pos.north())).withProperty((IProperty<Comparable>)BlockFire.EAST, this.canCatchFire(worldIn, pos.east())).withProperty((IProperty<Comparable>)BlockFire.SOUTH, this.canCatchFire(worldIn, pos.south())).withProperty((IProperty<Comparable>)BlockFire.WEST, this.canCatchFire(worldIn, pos.west())).withProperty((IProperty<Comparable>)BlockFire.UPPER, this.canCatchFire(worldIn, pos.up())) : this.getDefaultState();
    }
