...
public class BlockPortal extends BlockBreakable
{
    ...
    protected static final Box X_AABB;
    protected static final Box Z_AABB;
    protected static final Box Y_AABB;
    
    static {
        AXIS = PropertyEnum.create("axis", EnumFacing.Axis.class, EnumFacing.Axis.X, EnumFacing.Axis.Z);
        X_AABB = new Box(0.0, 0.0, 0.375, 1.0, 1.0, 0.625);
        Z_AABB = new Box(0.375, 0.0, 0.0, 0.625, 1.0, 1.0);
        Y_AABB = new Box(0.375, 0.0, 0.375, 0.625, 1.0, 0.625);
    }
    ...
    public void updateTick(final World worldIn, final BlockPos pos, final IBlockState state, final Random rand) {
        ...
        if (worldIn.provider.isSurfaceWorld() && worldIn.getGameRules().getBoolean("doMobSpawning") && rand.nextInt(2000) < worldIn.getDifficulty().getDifficultyId()) {
            ...
            if (i > 0 && !worldIn.getBlockState(blockpos.up()).isNormalCube()) {
                final Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.func_191306_a(EntityPigZombie.class), blockpos.getX() + 0.5, blockpos.getY() + 1.1, blockpos.getZ() + 0.5);
                if (entity != null) {
