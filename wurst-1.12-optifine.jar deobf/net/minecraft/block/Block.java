...
public class Block
{
    ...
    public static final Box FULL_BLOCK_AABB;
    @Nullable
    public static final Box NULL_AABB;
    ...
    static {
        ...
        FULL_BLOCK_AABB = new Box(0.0, 0.0, 0.0, 1.0, 1.0, 1.0);
        NULL_AABB = null;
    }
    ...
    public boolean isFullCube(final IBlockState state) {
        return !WurstClient.INSTANCE.hax.noClipMod.isActive();
    }
    ...
    public boolean shouldSideBeRendered(final IBlockState blockState, final IBlockAccess blockAccess, final BlockPos pos, final EnumFacing side) {
        final Box axisalignedbb = blockState.getBoundingBox(blockAccess, pos);
        switch (side) {
            case DOWN: {
    ...
    protected static void addCollisionBoxToList(final BlockPos pos, final Box entityBox, final List<Box> collidingBoxes, @Nullable final Box blockBox) {
        if (blockBox != Block.NULL_AABB) {
            final Box axisalignedbb = blockBox.offset(pos);
    ...
    public boolean canCollideCheck(final IBlockState state, final boolean hitIfLiquid) {
        final HandNoClipHack handNoClipHack = WurstClient.INSTANCE.hax.handNoClipHack;
        if (handNoClipHack.isActive()) {
            return handNoClipHack.isBlockInList(WBlock.getName(state.getBlock()));
        }
    ...
    public float getPlayerRelativeBlockHardness(final IBlockState state, final EntityPlayer player, final World worldIn, final BlockPos pos) {
        ...
        return (player.canHarvestBlock(state) ? (player.getDigSpeed(state) / f / 30.0f) : (player.getDigSpeed(state) / f / 100.0f)) * WurstClient.INSTANCE.hax.fastBreakMod.getHardnessModifier();
    }
    ...
    @Nullable
    protected RayTraceResult rayTrace(final BlockPos pos, final Vec3d start, final Vec3d end, final Box boundingBox) {
    ...
    public static void registerBlocks() {
        ...
        final Set<Block> set = Sets.newHashSet((Object[])new Block[] { Block.REGISTRY.getObject(new ResourceLocation("tripwire")) });
