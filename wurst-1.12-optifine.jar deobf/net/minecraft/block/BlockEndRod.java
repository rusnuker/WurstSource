...
{
    protected static final Box END_ROD_VERTICAL_AABB;
    protected static final Box END_ROD_NS_AABB;
    protected static final Box END_ROD_EW_AABB;
    
    static {
        END_ROD_VERTICAL_AABB = new Box(0.375, 0.0, 0.375, 0.625, 1.0, 0.625);
        END_ROD_NS_AABB = new Box(0.375, 0.375, 0.0, 0.625, 0.625, 1.0);
        END_ROD_EW_AABB = new Box(0.0, 0.375, 0.375, 1.0, 0.625, 0.625);
    }
    ...
    public void randomDisplayTick(final IBlockState stateIn, final World worldIn, final BlockPos pos, final Random rand) {
        ...
        if (rand.nextInt(5) == 0) {
            worldIn.spawnParticle(EnumParticleTypes.END_ROD, d0 + enumfacing.getFrontOffsetX() * d4, d2 + enumfacing.getFrontOffsetY() * d4, d3 + enumfacing.getFrontOffsetZ() * d4, rand.nextGaussian() * 0.005, rand.nextGaussian() * 0.005, rand.nextGaussian() * 0.005, new int[0]);
        }
    }
    ...
    public IBlockState getStateFromMeta(final int meta) {
        ...
        iblockstate = iblockstate.withProperty((IProperty<Comparable>)BlockEndRod.FACING, EnumFacing.getFront(meta));
        return iblockstate;
    }
