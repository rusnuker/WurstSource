...
public class CraftingManager
{
    ...
    private static boolean func_192420_c() {
        ...
        try {
                ...
                for (final Path path2 : Files.walk(path, new FileVisitOption[0])) {
                    if ("json".equals(FilenameUtils.getExtension(path2.toString()))) {
                        ...
                        try {
                            ...
                            CraftingManager.field_192422_a.error("Parsing error loading recipe " + resourcelocation, (Throwable)jsonparseexception);
                            final boolean flag3 = false;
                            return flag3;
                        }
                        catch (final IOException ioexception) {
                            CraftingManager.field_192422_a.error("Couldn't read recipe " + resourcelocation + " from " + path2, (Throwable)ioexception);
                            final boolean flag3 = false;
                            return flag3;
                        }
                        finally {
            ...
            CraftingManager.field_192422_a.error("Couldn't get a list of all recipe files", (Throwable)urisyntaxexception);
            flag4 = false;
            return flag4;
        }
        finally {
    ...
    public static NonNullList<ItemStack> getRemainingItems(final InventoryCrafting p_180303_0_, final World craftMatrix) {
        ...
        for (int i = 0; i < nonnulllist.size(); ++i) {
            nonnulllist.set(i, p_180303_0_.getInvStack(i));
        }
        return nonnulllist;
    }
