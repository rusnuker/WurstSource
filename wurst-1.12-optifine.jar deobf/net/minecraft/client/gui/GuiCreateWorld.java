...
public class GuiCreateWorld extends GuiScreen
{
    ...
    private static final String[] DISALLOWED_FILENAMES;
    ...
    public void initGui() {
        ...
        (this.worldNameField = new GuiTextField(9, this.fontRendererObj, this.width / 2 - 100, 60, 200, 20)).setFocused(true);
        this.worldNameField.setText(this.worldName);
        (this.worldSeedField = new GuiTextField(10, this.fontRendererObj, this.width / 2 - 100, 60, 200, 20)).setText(this.worldSeed);
    ...
    private void calcSaveDirName() {
        ...
        char[] illegal_FILE_CHARACTERS;
        for (int length = (illegal_FILE_CHARACTERS = ChatAllowedCharacters.ILLEGAL_FILE_CHARACTERS).length, i = 0; i < length; ++i) {
            final char c0 = illegal_FILE_CHARACTERS[i];
    ...
    public static String getUncollidingSaveDirName(final ISaveFormat saveLoader, String name) {
        ...
        String[] disallowed_FILENAMES;
        for (int length = (disallowed_FILENAMES = GuiCreateWorld.DISALLOWED_FILENAMES).length, i = 0; i < length; ++i) {
            final String s = disallowed_FILENAMES[i];
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        if (button.enabled) {
            if (button.id == 1) {
                ...
                final WorldSettings worldsettings = new WorldSettings(i, GameType.getByName(this.gameMode), this.generateStructuresEnabled, this.hardCoreMode, WorldType.WORLD_TYPES[this.selectedIndex]);
                ...
                if (WorldType.WORLD_TYPES[this.selectedIndex] == WorldType.FLAT) {
                    this.mc.openScreen(new GuiCreateFlatWorld(this, this.chunkProviderSettingsJson));
                }
                else {
                    this.mc.openScreen(new GuiCustomizeWorldScreen(this, this.chunkProviderSettingsJson));
                }
            }
        }
    }
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (this.inMoreWorldOptionsDisplay) {
            ...
            this.drawString(this.fontRendererObj, String.valueOf(I18n.format("selectWorld.resultFolder", new Object[0])) + " " + this.saveDirName, this.width / 2 - 100, 85, -6250336);
