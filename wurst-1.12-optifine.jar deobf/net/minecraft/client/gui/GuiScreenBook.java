...
public class GuiScreenBook extends GuiScreen
{
    ...
    public void signWithCommand(final String command) {
        try {
            this.command = command;
            this.sendBookToServer(this.hasCommand = true);
    ...
    public void initGui() {
        ...
        if (this.bookIsUnsigned) {
            ...
            this.buttonList.add(new GuiButton(6, (this.width + 192) / 2, 96, 100, 20, "Command Link"));
        }
        else {
    ...
    private void updateButtons() {
        ...
        if (this.bookIsUnsigned) {
            ...
            final GuiButton buttonCommandLink = this.buttonList.get(4);
            buttonCommandLink.visible = this.bookGettingSigned;
            buttonCommandLink.enabled = this.buttonFinalize.enabled;
        }
    }
    ...
    private void sendBookToServer(final boolean publish) throws IOException {
        if (this.bookIsUnsigned && this.bookIsModified && this.bookPages != null) {
            ...
            if (publish) {
                ...
                if (this.hasCommand) {
                    try {
                        final NBTTagList pages = this.bookObj.getTagCompound().getTagList("pages", 8);
                        for (int page = 0; page < pages.tagCount(); ++page) {
                            ...
                            component.setStyle(new Style()).getStyle().setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, this.command));
                            text = ITextComponent.Serializer.componentToJson(component);
                            pages.set(page, new NBTTagString(text));
                        }
                        System.out.println(this.bookObj.getTagCompound());
                    }
                    catch (final Exception e) {
                        e.printStackTrace();
                    }
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        if (button.enabled) {
            if (button.id == 0) {
                ...
                this.mc.openScreen(new GuiBookHack(this));
            }
            this.updateButtons();
        }
    }
