...
public class GuiWorldEdit extends GuiScreen
{
    ...
    public void initGui() {
        ...
        (this.nameEdit = new GuiTextField(2, this.fontRendererObj, this.width / 2 - 100, 60, 200, 20)).setFocused(true);
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        if (button.enabled) {
            if (button.id == 1) {
                ...
                final ISaveFormat isaveformat2 = this.mc.getSaveLoader();
                FileUtils.deleteQuietly(isaveformat2.getFile(this.worldId, "icon.png"));
                button.enabled = false;
            }
            else if (button.id == 4) {
                final ISaveFormat isaveformat3 = this.mc.getSaveLoader();
