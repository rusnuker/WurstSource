...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleJoinGame(final SPacketJoinGame packetIn) {
        ...
        this.gameController.openScreen(new GuiDownloadTerrain(this));
    ...
    public void handleHeldItemChange(final SPacketHeldItemChange packetIn) {
        ...
        if (InventoryPlayer.isHotbar(packetIn.getHeldItemHotbarIndex())) {
            this.gameController.player.inventory.selectedSlot = packetIn.getHeldItemHotbarIndex();
        }
    }
    ...
    public void handlePlayerPosLook(final SPacketPlayerPosLook packetIn) {
        ...
        if (!this.doneLoadingTerrain) {
            ...
            this.gameController.openScreen(null);
        }
    }
    ...
    public void onDisconnect(final ITextComponent reason) {
        ...
        if (this.guiScreenServer != null) {
            if (this.guiScreenServer instanceof GuiScreenRealmsProxy) {
                this.gameController.openScreen(new DisconnectedRealmsScreen(((GuiScreenRealmsProxy)this.guiScreenServer).getProxy(), "disconnect.lost", reason).getProxy());
            }
            else {
                this.gameController.openScreen(new GuiDisconnected(this.guiScreenServer, "disconnect.lost", reason));
            }
        }
        else {
            this.gameController.openScreen(new GuiDisconnected(new GuiMultiplayer(new GuiWurstMainMenu()), "disconnect.lost", reason));
        }
    }
    
    public void sendPacketBypass(final Packet packet) {
        this.netManager.sendPacket(packet);
    }
    
    public void sendPacket(final Packet<?> packetIn) {
        final PacketOutputListener.PacketOutputEvent event = new PacketOutputListener.PacketOutputEvent(packetIn);
    ...
    public void handleExplosion(final SPacketExplosion packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
        final Explosion explosion = new Explosion(this.gameController.world, null, packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getStrength(), packetIn.getAffectedBlockPositions());
    ...
    public void handleChangeGameState(final SPacketChangeGameState packetIn) {
        ...
        if (i == 1) {
            ...
            if (j == 0) {
                ...
                this.gameController.openScreen(new GuiWinGame(true, () -> this.gameController.player.connection.sendPacket(new CPacketClientStatus(CPacketClientStatus.State.PERFORM_RESPAWN))));
            }
        }
        else if (i == 5) {
            final GameSettings gamesettings = this.gameController.gameSettings;
            if (f == 0.0f) {
                this.gameController.openScreen(new GuiScreenDemo());
            }
            else if (f == 101.0f) {
    ...
    public void handleCombatEvent(final SPacketCombatEvent packetIn) {
        ...
        if (packetIn.eventType == SPacketCombatEvent.Event.ENTITY_DIED) {
            ...
            if (entity == this.gameController.player) {
                this.gameController.openScreen(new GuiGameOver(packetIn.deathMessage));
            }
        }
    }
    ...
    @Override
    public void handleNewKeepAlive(final SPacketNewKeepAlive packetIn) {
    ...
    public void handlePlayerAbilities(final SPacketPlayerAbilities packetIn) {
        ...
        entityplayer1.abilities.creativeMode = packetIn.isCreativeMode();
        entityplayer1.abilities.disableDamage = packetIn.isInvulnerable();
        entityplayer1.abilities.allowFlying = packetIn.isAllowFlying();
        entityplayer1.abilities.setFlySpeed(packetIn.getFlySpeed());
        entityplayer1.abilities.setPlayerWalkSpeed(packetIn.getWalkSpeed());
    }
            ...
            if (s.startsWith("level://")) {
                try {
                    ...
                    if (file2.isFile()) {
                        ...
                        Futures.addCallback((ListenableFuture)this.gameController.getResourcePackRepository().setResourcePackInstance(file2), (FutureCallback)this.createDownloadCallback());
                        return;
                    }
                }
                ...
                if (serverdata != null && serverdata.getResourceMode() == ServerData.ServerResourceMode.ENABLED) {
                    this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.ACCEPTED));
                    Futures.addCallback((ListenableFuture)this.gameController.getResourcePackRepository().downloadResourcePack(s, s2), (FutureCallback)this.createDownloadCallback());
                }
                    ...
                    this.gameController.addScheduledTask(new Runnable() {
                        @Override
                        public void run() {
                            NetHandlerPlayClient.this.gameController.openScreen(new GuiYesNo(new GuiYesNoCallback() {
                                @Override
                                public void confirmClicked(final boolean result, final int id) {
                                    ...
                                    if (result) {
                                        ...
                                        Futures.addCallback((ListenableFuture)NetHandlerPlayClient.this.gameController.getResourcePackRepository().downloadResourcePack(s, s2), NetHandlerPlayClient.this.createDownloadCallback());
                                    }
                                    else {
                                        if (serverdata1 != null) {
                                    ...
                                    NetHandlerPlayClient.this.gameController.openScreen(null);
                                }
    ...
    public void handleCustomPayload(final SPacketCustomPayload packetIn) {
        ...
        if ("MC|TrList".equals(packetIn.getChannelName())) {
            ...
            if (itemstack.getItem() == Items.WRITTEN_BOOK) {
                this.gameController.openScreen(new GuiScreenBook(this.gameController.player, itemstack, false));
            }
        }
