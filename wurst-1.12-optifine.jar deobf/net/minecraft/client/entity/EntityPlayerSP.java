...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate() {
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
            WurstClient.INSTANCE.events.fire(UpdateListener.UpdateEvent.INSTANCE);
            super.onUpdate();
            if (this.isRiding()) {
            ...
            WurstClient.INSTANCE.events.fire(PostUpdateListener.PostUpdateEvent.INSTANCE);
        }
    }
    ...
    private void onUpdateWalkingPlayer() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            final float yaw = RotationUtils.getServerYaw();
            final float pitch = RotationUtils.getServerPitch();
            final Box axisalignedbb = this.getEntityBoundingBox();
            ...
            if (!WurstClient.INSTANCE.hax.remoteViewMod.isActive() && !WurstClient.INSTANCE.hax.freecamMod.isActive() && !WurstClient.INSTANCE.hax.potionSaverMod.isFrozen()) {
                if (this.isRiding()) {
                    this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0, this.motionZ, yaw, pitch, this.onGround));
                    flag3 = false;
                }
                else if (flag3 && flag4) {
    ...
    public void sendChatMessage(final String message) {
        final ChatOutputListener.ChatOutputEvent event = new ChatOutputListener.ChatOutputEvent(message, false);
    ...
    public void sendAutomaticChatMessage(final String message) {
        final ChatOutputListener.ChatOutputEvent event = new ChatOutputListener.ChatOutputEvent(message, true);
        WurstClient.INSTANCE.commands.onSentMessage(event);
        WurstClient.INSTANCE.events.fire(event);
        if (event.isCancelled()) {
            return;
    ...
    public void openBook(final ItemStack stack, final EnumHand hand) {
        ...
        if (item == Items.WRITABLE_BOOK) {
            this.mc.openScreen(new GuiScreenBook(this, stack, true));
        }
    }
    ...
    public void displayGUIChest(final IInventory chestInventory) {
        ...
        if ("minecraft:chest".equals(s)) {
            this.mc.openScreen(new GuiChest(this.inventory, chestInventory));
        }
        else if ("minecraft:hopper".equals(s)) {
            this.mc.openScreen(new GuiHopper(this.inventory, chestInventory));
        }
        else if ("minecraft:furnace".equals(s)) {
            this.mc.openScreen(new GuiFurnace(this.inventory, chestInventory));
        }
        else if ("minecraft:brewing_stand".equals(s)) {
            this.mc.openScreen(new GuiBrewingStand(this.inventory, chestInventory));
        }
        else if ("minecraft:beacon".equals(s)) {
            this.mc.openScreen(new GuiBeacon(this.inventory, chestInventory));
        }
        else if (!"minecraft:dispenser".equals(s) && !"minecraft:dropper".equals(s)) {
            if ("minecraft:shulker_box".equals(s)) {
                this.mc.openScreen(new GuiShulkerBox(this.inventory, chestInventory));
            }
            else {
                this.mc.openScreen(new GuiChest(this.inventory, chestInventory));
            }
            ...
            this.mc.openScreen(new GuiDispenser(this.inventory, chestInventory));
        }
    }
    
    @Override
    public void openGuiHorseInventory(final AbstractHorse horse, final IInventory inventoryIn) {
        this.mc.openScreen(new GuiScreenHorseInventory(this.inventory, inventoryIn, horse));
    }
    ...
    public void displayGui(final IInteractionObject guiOwner) {
        ...
        if ("minecraft:crafting_table".equals(s)) {
            this.mc.openScreen(new GuiCrafting(this.inventory, this.world));
        }
        else if ("minecraft:enchanting_table".equals(s)) {
            this.mc.openScreen(new GuiEnchantment(this.inventory, this.world, guiOwner));
        }
        else if ("minecraft:anvil".equals(s)) {
            this.mc.openScreen(new GuiRepair(this.inventory, this.world));
        }
    }
    
    @Override
    public void displayVillagerTradeGui(final IMerchant villager) {
        this.mc.openScreen(new GuiMerchant(this.inventory, villager, this.world));
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.isHandActive() && !this.isRiding() && !WurstClient.INSTANCE.hax.noSlowdownMod.blockItemSlowness()) {
        ...
        this.pushOutOfBlocks(this.posX + this.width * 0.35, axisalignedbb.minY + 0.5, this.posZ - this.width * 0.35);
        this.pushOutOfBlocks(this.posX + this.width * 0.35, axisalignedbb.minY + 0.5, this.posZ + this.width * 0.35);
        final boolean flag5 = this.getFoodStats().getFoodLevel() > 6.0f || this.abilities.allowFlying;
        if (this.onGround && !flag2 && !flag3 && this.movementInput.moveForward >= 0.8f && !this.isSprinting() && flag5 && !this.isHandActive() && !this.isPotionActive(MobEffects.BLINDNESS)) {
            if (this.sprintToggleTimer <= 0 && !this.mc.gameSettings.keyBindSprint.isPressed()) {
        ...
        if (!this.isSprinting() && this.movementInput.moveForward >= 0.8f && flag5 && !this.isHandActive() && !this.isPotionActive(MobEffects.BLINDNESS) && this.mc.gameSettings.keyBindSprint.isPressed()) {
            this.setSprinting(true);
        }
        if (this.isSprinting() && (this.movementInput.moveForward < 0.8f || this.isCollidedHorizontally || !flag5)) {
        ...
        if (this.movementInput.jump && !flag && !this.onGround && this.motionY < 0.0 && !this.isElytraFlying() && !this.abilities.isFlying) {
    ...
    public boolean isAutoJumpEnabled() {
        return this.autoJumpEnabled && WurstClient.INSTANCE.hax.stepMod.isAutoJumpAllowed();
    }
                ...
                if (f8 >= -0.15f) {
                    ...
                    if (iblockstate.getCollisionBoundingBox(this.world, blockpos) == null) {
                        ...
                        if (iblockstate2.getCollisionBoundingBox(this.world, blockpos) == null) {
                            ...
                            final Box axisalignedbb = new Box(vec3d, vec3d6.addVector(0.0, f13, 0.0)).expand(f12, 0.0, f12);
                            ...
                            final List<Box> list = this.world.getCollisionBoxes(this, axisalignedbb);
                            if (!list.isEmpty()) {}
                            float f14 = Float.MIN_VALUE;
                            for (final Box axisalignedbb2 : list) {
    ...
    public boolean isInWater() {
        final boolean inWater = super.isInWater();
        final IsPlayerInWaterListener.IsPlayerInWaterEvent event = new IsPlayerInWaterListener.IsPlayerInWaterEvent(inWater);
