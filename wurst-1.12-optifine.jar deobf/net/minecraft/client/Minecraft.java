...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    static {
        ...
        LOCATION_MOJANG_PNG = new ResourceLocation("wurst/wurst_loading.png");
        ...
        try {
            do {
                ...
                try {
                    ...
                    this.freeMemory();
                    this.openScreen(new GuiMemoryErrorScreen());
                    System.gc();
                }
            } while (this.running);
        }
        catch (final MinecraftError var11) {}
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        if (this.serverName != null) {
            this.openScreen(new GuiConnecting(new GuiWurstMainMenu(), this, this.serverName, this.serverPort));
        }
        else {
            this.openScreen(new GuiWurstMainMenu());
        }
    ...
    private void setInitialDisplayMode() throws LWJGLException {
        if (this.fullscreen) {
            ...
            FrameHook.createFrame(this.mcDefaultResourcePack, Minecraft.LOGGER);
        }
    }
    ...
    public void refreshResources() {
        final List<IResourcePack> list = Lists.newArrayList((Iterable)this.defaultResourcePacks);
    ...
    public void openScreen(@Nullable GuiScreen guiScreenIn) {
    ...
    private void runGameLoop() throws IOException {
        ...
        ++this.fpsCounter;
        final boolean flag = this.isSingleplayer() && this.currentScreen != null && this.currentScreen.doesGuiPauseGame() && !this.theIntegratedServer.getPublic();
    ...
    public void displayInGameMenu() {
        if (this.currentScreen == null) {
            this.openScreen(new GuiIngameMenu());
    ...
    private void clickMouse() {
        if (this.leftClickCounter <= 0) {
            final LeftClickListener.LeftClickEvent event = new LeftClickListener.LeftClickEvent();
    ...
    public void rightClickMouse() {
        if (!this.playerController.getIsHittingBlock()) {
            final RightClickListener.RightClickEvent event = new RightClickListener.RightClickEvent();
            WurstClient.INSTANCE.events.fire(event);
            if (event.isCancelled()) {
                return;
            }
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null && this.player != null) {
            if (this.player.getHealth() <= 0.0f && !(this.currentScreen instanceof GuiGameOver)) {
                ...
                this.openScreen(new GuiSleepMP());
            }
        }
        ...
        if (this.world != null) {
            ...
            if (!this.isGamePaused && this.world != null && !WurstClient.INSTANCE.hax.freecamMod.isActive()) {
    ...
    private void processKeyBinds() {
        while (this.gameSettings.keyBindTogglePerspective.wasPressed()) {
        ...
        while (this.gameSettings.keyBindSmoothCamera.wasPressed()) {
            this.gameSettings.smoothCamera = !this.gameSettings.smoothCamera;
        }
        for (int i = 0; i < 9; ++i) {
            final boolean flag = this.gameSettings.field_193629_ap.isPressed();
            final boolean flag2 = this.gameSettings.field_193630_aq.isPressed();
            if (this.gameSettings.keyBindsHotbar[i].wasPressed()) {
                if (this.player.isSpectator()) {
                    this.ingameGUI.getSpectatorGui().onHotbarSelected(i);
                }
                else if (!this.player.isCreative() || this.currentScreen != null || (!flag2 && !flag)) {
        ...
        while (this.gameSettings.keyBindInventory.wasPressed()) {
            if (this.playerController.isRidingHorse()) {
                this.player.sendHorseInventory();
            }
            else {
                this.field_193035_aW.func_193296_a();
                this.openScreen(new GuiInventory(this.player));
            }
        }
        while (this.gameSettings.field_194146_ao.wasPressed()) {
            this.openScreen(new GuiScreenAdvancements(this.player.connection.func_191982_f()));
        }
        while (this.gameSettings.keyBindSwapHands.wasPressed()) {
            if (!this.player.isSpectator()) {
        ...
        while (this.gameSettings.keyBindDrop.wasPressed()) {
            if (!this.player.isSpectator()) {
        ...
        if (flag3) {
            while (this.gameSettings.keyBindChat.wasPressed()) {
                this.openScreen(new GuiChat());
            }
            if (this.currentScreen == null && this.gameSettings.keyBindCommand.wasPressed()) {
                this.openScreen(new GuiChat("/"));
            }
        }
        if (this.player.isHandActive()) {
            ...
            while (this.gameSettings.keyBindAttack.wasPressed()) {}
            while (this.gameSettings.keyBindUseItem.wasPressed()) {}
            while (this.gameSettings.keyBindPickBlock.wasPressed()) {}
        }
        else {
            while (this.gameSettings.keyBindAttack.wasPressed()) {
                this.clickMouse();
        ...
        if (this.gameSettings.keyBindUseItem.isPressed() && this.rightClickDelayTimer == 0 && !this.player.isHandActive()) {
            this.rightClickMouse();
        }
        this.sendClickBlockToController(this.currentScreen == null && this.gameSettings.keyBindAttack.isPressed() && this.inGameHasFocus);
    }
                ...
                if (k != 0) {
                    if (this.player.isSpectator()) {
                        ...
                        if (this.ingameGUI.getSpectatorGui().isMenuActive()) {
                            ...
                            final float f = MathHelper.clamp(this.player.abilities.getFlySpeed() + k * 0.005f, 0.0f, 0.2f);
    ...
    public void launchIntegratedServer(final String folderName, final String worldName, @Nullable WorldSettings worldSettingsIn) {
        ...
        this.openScreen(new GuiScreenWorking());
    ...
    private void middleClickMouse() {
        if (this.objectMouseOver != null && this.objectMouseOver.typeOfHit != RayTraceResult.Type.MISS) {
            final boolean flag = this.player.abilities.creativeMode;
            ...
            if (this.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                WurstClient.INSTANCE.friends.middleClick(this.objectMouseOver.entityHit);
            ...
            if (itemstack.func_190926_b()) {
                ...
                if (flag) {
                    ...
                    this.playerController.sendSlotPacket(this.player.getHeldItem(EnumHand.MAIN_HAND), 36 + inventoryplayer.selectedSlot);
                }
                else if (i != -1) {
                    if (InventoryPlayer.isHotbar(i)) {
                        inventoryplayer.selectedSlot = i;
                    }
                    else {
    ...
    public MusicTicker.MusicType getAmbientMusicType() {
        ...
        return (this.player.abilities.creativeMode && this.player.abilities.allowFlying) ? MusicTicker.MusicType.CREATIVE : MusicTicker.MusicType.GAME;
    }
