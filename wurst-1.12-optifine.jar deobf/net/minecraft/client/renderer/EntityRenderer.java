...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(final float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            for (int j = 0; j < list.size(); ++j) {
                ...
                final Box axisalignedbb = entity2.getEntityBoundingBox().expandXyz(entity2.getCollisionBorderSize());
    ...
    private void hurtCameraEffect(final float partialTicks) {
        if (WurstClient.INSTANCE.hax.noHurtcamMod.isActive()) {
            return;
        }
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (raytraceresult != null && !WurstClient.INSTANCE.hax.cameraNoClipMod.isActive()) {
    ...
    public void setupCameraTransform(final float partialTicks, final int pass) {
        ...
        if (this.mc.gameSettings.viewBobbing && EventFactory.cameraTransformViewBobbing()) {
        ...
        if (f2 > 0.0f && !WurstClient.INSTANCE.hax.antiWobbleHack.isActive()) {
            int i = 20;
    ...
    public void renderWorld(final float partialTicks, final long finishTimeNano) {
        ...
        if (!WurstClient.INSTANCE.hax.caveFinderMod.isActive()) {
    ...
    private void renderWorldPass(final int pass, final float partialTicks, final long finishTimeNano) {
        ...
        if (!Config.isShaders()) {
            WurstClient.INSTANCE.events.fire(new RenderListener.RenderEvent(partialTicks));
        }
        ...
        if (f != 0.0f && Config.isRainSplash()) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (blockpos2.getY() <= blockpos.getY() + 10 && blockpos2.getY() >= blockpos.getY() - 10 && biome.canRain() && biome.getFloatTemperature(blockpos2) >= 0.15f) {
                    ...
                    final Box axisalignedbb = iblockstate.getBoundingBox(world, blockpos3);
    ...
    private void updateFogColor(final float partialTicks) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS) && !WurstClient.INSTANCE.hax.antiBlindMod.isActive()) {
    ...
    private void setupFog(final int startCoords, final float partialTicks) {
        ...
        if (f >= 0.0f) {
        ...
        else if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS) && !WurstClient.INSTANCE.hax.antiBlindMod.isActive()) {
            float f2 = 5.0f;
            final int i = ((EntityLivingBase)entity).getActivePotionEffect(MobEffects.BLINDNESS).getDuration();
            if (i < 20) {
            ...
            if (entity instanceof EntityLivingBase) {
                if (((EntityLivingBase)entity).isPotionActive(MobEffects.WATER_BREATHING) || WurstClient.INSTANCE.hax.noOverlayMod.isActive()) {
            ...
            if (WurstClient.INSTANCE.hax.noOverlayMod.isActive()) {
                GlStateManager.setFogDensity(0.01f);
            }
            else {
                if (Config.isShaders()) {
    ...
    public static void drawNameplate(final FontRenderer fontRendererIn, final String str, final float x, final float y, final float z, final int verticalShift, final float viewerYaw, final float viewerPitch, final boolean isThirdPersonFrontal, boolean isSneaking) {
        GlStateManager.pushMatrix();
        ...
        float scale = 0.025f;
        if (WurstClient.INSTANCE.hax.nameTagsMod.isActive()) {
            isSneaking = false;
            final double distance = Math.sqrt(x * x + y * y + z * z);
            if (distance > 10.0) {
                scale *= (float)(distance / 10.0);
            }
        }
