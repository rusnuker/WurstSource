...
public class RenderGlobal implements IWorldEventListener, IResourceManagerReloadListener
{
    ...
    public void renderEntities(final Entity renderViewEntity, final ICamera camera, final float partialTicks) {
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            final List<Entity> list2 = Lists.newArrayList();
            ...
            for (final ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfosEntities) {
                final Chunk chunk = renderglobal$containerlocalrenderinformation.renderChunk.getChunk(this.theWorld);
                ...
                if (!list4.isEmpty()) {
                    for (final TileEntity tileentity1 : list4) {
                        if (flag2) {
                            ...
                            final Box axisalignedbb = (Box)Reflector.call(tileentity1, Reflector.ForgeTileEntity_getRenderBoundingBox, new Object[0]);
    ...
    private boolean isOutlineActive(final Entity p_184383_1_, final Entity p_184383_2_, final ICamera p_184383_3_) {
        ...
        return (p_184383_1_ != p_184383_2_ || this.mc.gameSettings.thirdPersonView != 0 || flag) && (p_184383_1_.isGlowing() || (this.mc.player.isSpectator() && this.mc.gameSettings.keyBindSpectatorOutlines.isPressed() && p_184383_1_ instanceof EntityPlayer && (p_184383_1_.ignoreFrustumCheck || p_184383_3_.isBoundingBoxInFrustum(p_184383_1_.getEntityBoundingBox()) || p_184383_1_.isRidingOrBeingRiddenBy(this.mc.player))));
    }
    ...
    public void renderSky(final float partialTicks, final int pass) {
        ...
        if (this.mc.world.provider.getDimensionType() == DimensionType.THE_END) {
            ...
            if (afloat != null && Config.isSunMoonEnabled()) {
                ...
                final int j = 16;
                for (int l = 0; l <= 16; ++l) {
                    final float f13 = l * 6.2831855f / 16.0f;
            ...
            final double d0 = this.mc.player.getPositionEyes(partialTicks).yCoord - this.theWorld.getHorizon();
            if (d0 < 0.0) {
                GlStateManager.pushMatrix();
            ...
            if (this.mc.world.provider.isSurfaceWorld()) {
                ...
                if (Config.isCloudsFancy()) {
                    ...
                    if (this.cloudRenderer.shouldUpdateGlList()) {
                        ...
                        double d3 = p_180447_3_ + d2 * 0.029999999329447746;
                        final int k = MathHelper.floor(d3 / 2048.0);
                        final int l = MathHelper.floor(p_180447_7_ / 2048.0);
                        d3 -= k * 2048;
                        final double d4 = p_180447_7_ - l * 2048;
                        ...
                        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                        for (int m = -256; m < 256; m += 32) {
                            for (int i2 = -256; i2 < 256; i2 += 32) {
                                bufferbuilder.pos(m + 0, f17, i2 + 32).tex((m + 0) * 4.8828125E-4f + f18, (i2 + 32) * 4.8828125E-4f + f19).color(f10, f11, f12, 0.8f).endVertex();
                                bufferbuilder.pos(m + 32, f17, i2 + 32).tex((m + 32) * 4.8828125E-4f + f18, (i2 + 32) * 4.8828125E-4f + f19).color(f10, f11, f12, 0.8f).endVertex();
                                bufferbuilder.pos(m + 32, f17, i2 + 0).tex((m + 32) * 4.8828125E-4f + f18, (i2 + 0) * 4.8828125E-4f + f19).color(f10, f11, f12, 0.8f).endVertex();
                                bufferbuilder.pos(m + 0, f17, i2 + 0).tex((m + 0) * 4.8828125E-4f + f18, (i2 + 0) * 4.8828125E-4f + f19).color(f10, f11, f12, 0.8f).endVertex();
                            }
                        }
                        tessellator.draw();
    ...
    private void renderCloudsFancy(final float partialTicks, final int pass, final double p_180445_3_, final double p_180445_5_, final double p_180445_7_) {
        ...
        final double d0 = this.cloudTickCounter + f251;
        double d2 = (p_180445_3_ + d0 * 0.029999999329447746) / 12.0;
        double d3 = p_180445_7_ / 12.0 + 0.33000001311302185;
        float f254 = this.theWorld.provider.getCloudHeight() - (float)p_180445_5_ + 0.33f;
        f254 += this.mc.gameSettings.ofCloudsHeight * 128.0f;
        final int i = MathHelper.floor(d2 / 2048.0);
        final int j = MathHelper.floor(d3 / 2048.0);
        d2 -= i * 2048;
        d3 -= j * 2048;
        this.renderEngine.bindTexture(RenderGlobal.CLOUDS_TEXTURES);
        GlStateManager.enableBlend();
        ...
        final float f271 = MathHelper.floor(d2) * 0.00390625f;
        final float f272 = MathHelper.floor(d3) * 0.00390625f;
        final float f273 = (float)(d2 - MathHelper.floor(d2));
        final float f274 = (float)(d3 - MathHelper.floor(d3));
        final int k = 8;
        final int l = 4;
        ...
        if (this.cloudRenderer.shouldUpdateGlList()) {
            ...
            for (int l2 = -3; l2 <= 4; ++l2) {
                for (int j2 = -3; j2 <= 4; ++j2) {
                    ...
                    final float f276 = (float)(l2 * 8);
                    final float f277 = (float)(j2 * 8);
                    ...
                    if (l2 > -1) {
                        for (int k2 = 0; k2 < 8; ++k2) {
                            bufferbuilder.pos(f278 + k2 + 0.0f, f254 + 0.0f, f279 + 8.0f).tex((f276 + k2 + 0.5f) * 0.00390625f + f271, (f277 + 8.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(-1.0f, 0.0f, 0.0f).endVertex();
                            bufferbuilder.pos(f278 + k2 + 0.0f, f254 + 4.0f, f279 + 8.0f).tex((f276 + k2 + 0.5f) * 0.00390625f + f271, (f277 + 8.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(-1.0f, 0.0f, 0.0f).endVertex();
                            bufferbuilder.pos(f278 + k2 + 0.0f, f254 + 4.0f, f279 + 0.0f).tex((f276 + k2 + 0.5f) * 0.00390625f + f271, (f277 + 0.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(-1.0f, 0.0f, 0.0f).endVertex();
                            bufferbuilder.pos(f278 + k2 + 0.0f, f254 + 0.0f, f279 + 0.0f).tex((f276 + k2 + 0.5f) * 0.00390625f + f271, (f277 + 0.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(-1.0f, 0.0f, 0.0f).endVertex();
                        }
                    }
                    if (l2 <= 1) {
                        for (int i3 = 0; i3 < 8; ++i3) {
                            bufferbuilder.pos(f278 + i3 + 1.0f - 9.765625E-4f, f254 + 0.0f, f279 + 8.0f).tex((f276 + i3 + 0.5f) * 0.00390625f + f271, (f277 + 8.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(1.0f, 0.0f, 0.0f).endVertex();
                            bufferbuilder.pos(f278 + i3 + 1.0f - 9.765625E-4f, f254 + 4.0f, f279 + 8.0f).tex((f276 + i3 + 0.5f) * 0.00390625f + f271, (f277 + 8.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(1.0f, 0.0f, 0.0f).endVertex();
                            bufferbuilder.pos(f278 + i3 + 1.0f - 9.765625E-4f, f254 + 4.0f, f279 + 0.0f).tex((f276 + i3 + 0.5f) * 0.00390625f + f271, (f277 + 0.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(1.0f, 0.0f, 0.0f).endVertex();
                            bufferbuilder.pos(f278 + i3 + 1.0f - 9.765625E-4f, f254 + 0.0f, f279 + 0.0f).tex((f276 + i3 + 0.5f) * 0.00390625f + f271, (f277 + 0.0f) * 0.00390625f + f272).color(f261, f262, f263, 0.8f).normal(1.0f, 0.0f, 0.0f).endVertex();
                        }
                    }
                    if (j2 > -1) {
                        for (int j3 = 0; j3 < 8; ++j3) {
                            bufferbuilder.pos(f278 + 0.0f, f254 + 4.0f, f279 + j3 + 0.0f).tex((f276 + 0.0f) * 0.00390625f + f271, (f277 + j3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, -1.0f).endVertex();
                            bufferbuilder.pos(f278 + 8.0f, f254 + 4.0f, f279 + j3 + 0.0f).tex((f276 + 8.0f) * 0.00390625f + f271, (f277 + j3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, -1.0f).endVertex();
                            bufferbuilder.pos(f278 + 8.0f, f254 + 0.0f, f279 + j3 + 0.0f).tex((f276 + 8.0f) * 0.00390625f + f271, (f277 + j3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, -1.0f).endVertex();
                            bufferbuilder.pos(f278 + 0.0f, f254 + 0.0f, f279 + j3 + 0.0f).tex((f276 + 0.0f) * 0.00390625f + f271, (f277 + j3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, -1.0f).endVertex();
                        }
                    }
                    if (j2 <= 1) {
                        for (int k3 = 0; k3 < 8; ++k3) {
                            bufferbuilder.pos(f278 + 0.0f, f254 + 4.0f, f279 + k3 + 1.0f - 9.765625E-4f).tex((f276 + 0.0f) * 0.00390625f + f271, (f277 + k3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, 1.0f).endVertex();
                            bufferbuilder.pos(f278 + 8.0f, f254 + 4.0f, f279 + k3 + 1.0f - 9.765625E-4f).tex((f276 + 8.0f) * 0.00390625f + f271, (f277 + k3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, 1.0f).endVertex();
                            bufferbuilder.pos(f278 + 8.0f, f254 + 0.0f, f279 + k3 + 1.0f - 9.765625E-4f).tex((f276 + 8.0f) * 0.00390625f + f271, (f277 + k3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, 1.0f).endVertex();
                            bufferbuilder.pos(f278 + 0.0f, f254 + 0.0f, f279 + k3 + 1.0f - 9.765625E-4f).tex((f276 + 0.0f) * 0.00390625f + f271, (f277 + k3 + 0.5f) * 0.00390625f + f272).color(f267, f268, f269, 0.8f).normal(0.0f, 0.0f, 1.0f).endVertex();
                        }
                    }
                    tessellator.draw();
                }
        ...
        if (!this.chunksToUpdate.isEmpty()) {
            ...
            while (iterator3.hasNext()) {
                ...
                if (renderchunk3.isNeedsUpdateCustom()) {
                    flag = this.renderDispatcher.updateChunkNow(renderchunk3);
                }
                else {
                    flag = this.renderDispatcher.updateChunkLater(renderchunk3);
                }
                if (!flag) {
                    break;
                }
    ...
    public void renderWorldBorder(final Entity entityIn, final float partialTicks) {
        ...
        if (entityIn.posX >= worldborder.maxX() - d0 || entityIn.posX <= worldborder.minX() + d0 || entityIn.posZ >= worldborder.maxZ() - d0 || entityIn.posZ <= worldborder.minZ() + d0) {
            double d2 = 1.0 - worldborder.getClosestDistance(entityIn) / d0;
            d2 = Math.pow(d2, 4.0);
            ...
            double d6 = Math.max(MathHelper.floor(d5 - d0), worldborder.minZ());
            double d7 = Math.min(MathHelper.ceil(d5 + d0), worldborder.maxZ());
            if (d3 > worldborder.maxX() - d0) {
                float f8 = 0.0f;
                for (double d8 = d6; d8 < d7; ++d8, f8 += 0.5f) {
                    final double d9 = Math.min(1.0, d7 - d8);
                    final float f9 = (float)d9 * 0.5f;
                    bufferbuilder.pos(worldborder.maxX(), 256.0, d8).tex(f4 + f8, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(worldborder.maxX(), 256.0, d8 + d9).tex(f4 + f9 + f8, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(worldborder.maxX(), 0.0, d8 + d9).tex(f4 + f9 + f8, f4 + 128.0f).endVertex();
                    bufferbuilder.pos(worldborder.maxX(), 0.0, d8).tex(f4 + f8, f4 + 128.0f).endVertex();
                }
            }
            if (d3 < worldborder.minX() + d0) {
                ...
                for (double d10 = d6; d10 < d7; ++d10, f10 += 0.5f) {
                    ...
                    bufferbuilder.pos(worldborder.minX(), 256.0, d10).tex(f4 + f10, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(worldborder.minX(), 256.0, d10 + d11).tex(f4 + f11 + f10, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(worldborder.minX(), 0.0, d10 + d11).tex(f4 + f11 + f10, f4 + 128.0f).endVertex();
                    bufferbuilder.pos(worldborder.minX(), 0.0, d10).tex(f4 + f10, f4 + 128.0f).endVertex();
                }
            }
            d6 = Math.max(MathHelper.floor(d3 - d0), worldborder.minX());
            d7 = Math.min(MathHelper.ceil(d3 + d0), worldborder.maxX());
            if (d5 > worldborder.maxZ() - d0) {
                ...
                for (double d12 = d6; d12 < d7; ++d12, f12 += 0.5f) {
                    ...
                    bufferbuilder.pos(d12, 256.0, worldborder.maxZ()).tex(f4 + f12, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(d12 + d13, 256.0, worldborder.maxZ()).tex(f4 + f13 + f12, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(d12 + d13, 0.0, worldborder.maxZ()).tex(f4 + f13 + f12, f4 + 128.0f).endVertex();
                    bufferbuilder.pos(d12, 0.0, worldborder.maxZ()).tex(f4 + f12, f4 + 128.0f).endVertex();
                }
            }
            if (d5 < worldborder.minZ() + d0) {
                ...
                for (double d14 = d6; d14 < d7; ++d14, f14 += 0.5f) {
                    ...
                    bufferbuilder.pos(d14, 256.0, worldborder.minZ()).tex(f4 + f14, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(d14 + d15, 256.0, worldborder.minZ()).tex(f4 + f15 + f14, f4 + 0.0f).endVertex();
                    bufferbuilder.pos(d14 + d15, 0.0, worldborder.minZ()).tex(f4 + f15 + f14, f4 + 128.0f).endVertex();
                    bufferbuilder.pos(d14, 0.0, worldborder.minZ()).tex(f4 + f14, f4 + 128.0f).endVertex();
                }
            }
            tessellator.draw();
    ...
    public void drawSelectionBox(final EntityPlayer player, final RayTraceResult movingObjectPositionIn, final int execute, final float partialTicks) {
        if (execute == 0 && movingObjectPositionIn.typeOfHit == RayTraceResult.Type.BLOCK) {
            ...
            if (iblockstate.getMaterial() != Material.AIR && this.theWorld.getWorldBorder().contains(blockpos)) {
                ...
                drawSelectionBoundingBox(iblockstate.getSelectedBoundingBox(this.theWorld, blockpos).expandXyz(0.0020000000949949026).offset(-d0, -d2, -d3), 0.0f, 0.0f, 0.0f, 0.4f);
            }
            GlStateManager.depthMask(true);
            GlStateManager.enableTexture2D();
            if (Config.isShaders()) {
    ...
    public static void drawSelectionBoundingBox(final Box box, final float red, final float green, final float blue, final float alpha) {
    ...
    public static void renderFilledBox(final Box p_189696_0_, final float p_189696_1_, final float p_189696_2_, final float p_189696_3_, final float p_189696_4_) {
    ...
    public void notifyBlockUpdate(final World worldIn, final BlockPos pos, final IBlockState oldState, final IBlockState newState, final int flags) {
        ...
        this.markBlocksForUpdate(i - 1, j - 1, k - 1, i + 1, j + 1, k + 1, (flags & 0x8) != 0x0);
    }
    ...
    private void func_193054_a(final World p_193054_1_, final BlockPos p_193054_2_, final boolean p_193054_3_) {
        for (final EntityLivingBase entitylivingbase : p_193054_1_.getEntitiesWithinAABB((Class<? extends EntityLivingBase>)EntityLivingBase.class, new Box(p_193054_2_).expandXyz(3.0))) {
    ...
    private Particle func_190571_b(final int p_190571_1_, final boolean p_190571_2_, final boolean p_190571_3_, final double p_190571_4_, final double p_190571_6_, final double p_190571_8_, final double p_190571_10_, final double p_190571_12_, final double p_190571_14_, final int... p_190571_16_) {
        ...
        final int i = this.func_190572_a(p_190571_3_);
        final double d0 = entity.posX - p_190571_4_;
        final double d2 = entity.posY - p_190571_6_;
        final double d3 = entity.posZ - p_190571_8_;
    ...
    public void broadcastSound(final int soundID, final BlockPos pos, final int data) {
        switch (soundID) {
            ...
            case 1038: {
                ...
                final double d0 = pos.getX() - entity.posX;
                final double d2 = pos.getY() - entity.posY;
                final double d3 = pos.getZ() - entity.posZ;
                ...
                if (soundID == 1023) {
                    this.theWorld.playSound(d5, d6, d7, SoundEvents.ENTITY_WITHER_SPAWN, SoundCategory.HOSTILE, 1.0f, 1.0f, false);
                    break;
                }
                if (soundID == 1038) {
                    this.theWorld.playSound(d5, d6, d7, SoundEvents.field_193782_bq, SoundCategory.HOSTILE, 1.0f, 1.0f, false);
                    break;
                }
                this.theWorld.playSound(d5, d6, d7, SoundEvents.ENTITY_ENDERDRAGON_DEATH, SoundCategory.HOSTILE, 5.0f, 1.0f, false);
                break;
            }
        }
    }
    ...
    public void playEvent(final EntityPlayer player, final int type, final BlockPos blockPosIn, final int data) {
        ...
        switch (type) {
            ...
            case 2007: {
                ...
                for (int k = 0; k < 8; ++k) {
                    this.spawnParticle(EnumParticleTypes.ITEM_CRACK, d11, d12, d13, random.nextGaussian() * 0.15, random.nextDouble() * 0.2, random.nextGaussian() * 0.15, Item.getIdFromItem(Items.SPLASH_POTION));
                }
                ...
                for (int j2 = 0; j2 < 100; ++j2) {
                    ...
                    final Particle particle1 = this.spawnEntityFX(enumparticletypes.getParticleID(), enumparticletypes.getShouldIgnoreRange(), d11 + d16 * 0.1, d12 + 0.3, d13 + d18 * 0.1, d16, d17, d18, new int[0]);
                    if (particle1 != null) {
                        final float f8 = 0.75f + random.nextFloat() * 0.25f;
                        particle1.setRBGColorF(f5 * f8, f6 * f8, f7 * f8);
                        particle1.multiplyVelocity((float)d14);
                    }
                }
            ...
            case 2006: {
                for (int m = 0; m < 200; ++m) {
                    ...
                    final double d26 = MathHelper.cos(f10) * f9;
                    final double d27 = 0.01 + random.nextDouble() * 0.5;
                    final double d28 = MathHelper.sin(f10) * f9;
                    final Particle particle2 = this.spawnEntityFX(EnumParticleTypes.DRAGON_BREATH.getParticleID(), false, blockPosIn.getX() + d26 * 0.1, blockPosIn.getY() + 0.3, blockPosIn.getZ() + d28 * 0.1, d26, d27, d28, new int[0]);
                    if (particle2 != null) {
    ...
    private ContainerLocalRenderInformation allocateRenderInformation(final RenderChunk p_allocateRenderInformation_1_, final EnumFacing p_allocateRenderInformation_2_, final int p_allocateRenderInformation_3_) {
        ...
        if (RenderGlobal.renderInfoCache.isEmpty()) {
            ...
            renderglobal$containerlocalrenderinformation = RenderGlobal.renderInfoCache.pollLast();
    ...
    public static class ContainerLocalRenderInformation
    {
        ...
        public ContainerLocalRenderInformation(final RenderChunk p_i3_1_, final EnumFacing p_i3_2_, final int p_i3_3_) {
