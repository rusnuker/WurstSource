...
public abstract class Render<T extends Entity> implements IEntityRenderer
{
    ...
    public boolean shouldRender(final T livingEntity, final ICamera camera, final double camX, final double camY, final double camZ) {
        Box axisalignedbb = livingEntity.getRenderBoundingBox().expandXyz(0.5);
        if (axisalignedbb.hasNaN() || axisalignedbb.getAverageEdgeLength() == 0.0) {
            axisalignedbb = new Box(livingEntity.posX - 2.0, livingEntity.posY - 2.0, livingEntity.posZ - 2.0, livingEntity.posX + 2.0, livingEntity.posY + 2.0, livingEntity.posZ + 2.0);
        }
    ...
    private void renderShadowSingle(final IBlockState state, final double p_188299_2_, final double p_188299_4_, final double p_188299_6_, final BlockPos p_188299_8_, final float p_188299_9_, final float p_188299_10_, final double p_188299_11_, final double p_188299_13_, final double p_188299_15_) {
        if (state.isFullCube()) {
            ...
            if (d0 >= 0.0) {
                ...
                final Box axisalignedbb = state.getBoundingBox(this.getWorldFromRenderManager(), p_188299_8_);
    ...
    public static void renderOffsetAABB(final Box boundingBox, final double x, final double y, final double z) {
    ...
    protected void renderLivingLabel(final T entityIn, String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(this.renderManager.renderViewEntity);
        if (d0 <= maxDistance * maxDistance) {
            ...
            if (entityIn != null && WurstClient.INSTANCE.friends.contains(entityIn.getName())) {
                str = "\ufffd2" + (str.startsWith("\ufffd") ? str.substring(2) : str);
            }
            if (entityIn instanceof EntityLivingBase && WurstClient.INSTANCE.hax.healthTagsMod.isActive()) {
    ...
    public void renderMultipass(final T p_188300_1_, final double p_188300_2_, final double p_188300_4_, final double p_188300_6_, final float p_188300_8_, final float p_188300_9_) {
    }
