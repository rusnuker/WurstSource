...
public class RenderShulker extends RenderLiving<EntityShulker>
{
    ...
    public boolean shouldRender(final EntityShulker livingEntity, final ICamera camera, final double camX, final double camY, final double camZ) {
        ...
        if (livingEntity.getClientTeleportInterp() > 0 && livingEntity.isAttachedToBlock()) {
            ...
            if (camera.isBoundingBoxInFrustum(new Box(vec3d2.xCoord, vec3d2.yCoord, vec3d2.zCoord, vec3d.xCoord, vec3d.yCoord, vec3d.zCoord))) {
                return true;
            }
        }
        return false;
    ...
    protected ResourceLocation getEntityTexture(final EntityShulker entity) {
        return RenderShulker.SHULKER_ENDERGOLEM_TEXTURE[entity.func_190769_dn().getMetadata()];
    }
    
    @Override
    protected void rotateCorpse(final EntityShulker entityLiving, final float p_77043_2_, final float p_77043_3_, final float partialTicks) {
