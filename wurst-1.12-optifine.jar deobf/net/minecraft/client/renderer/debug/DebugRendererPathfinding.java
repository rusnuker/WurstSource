...
public class DebugRendererPathfinding implements DebugRenderer.IDebugRenderer
{
    ...
    public DebugRendererPathfinding(final Minecraft minecraftIn) {
        ...
        this.pathMaxDistance = Maps.newHashMap();
        ...
        if (!this.pathMap.isEmpty()) {
            ...
            for (final Integer integer : this.pathMap.keySet()) {
                ...
                if (this.addDistanceToPlayer(pathpoint) <= 40.0f) {
                    RenderGlobal.renderFilledBox(new Box(pathpoint.xCoord + 0.25f, pathpoint.yCoord + 0.25f, pathpoint.zCoord + 0.25, pathpoint.xCoord + 0.75f, pathpoint.yCoord + 0.75f, pathpoint.zCoord + 0.75f).offset(-this.xo, -this.yo, -this.zo), 0.0f, 1.0f, 0.0f, 0.5f);
                    for (int j = 0; j < path.getCurrentPathLength(); ++j) {
                        ...
                        if (this.addDistanceToPlayer(pathpoint2) <= 40.0f) {
                            ...
                            RenderGlobal.renderFilledBox(new Box(pathpoint2.xCoord + 0.5f - f, pathpoint2.yCoord + 0.01f * j, pathpoint2.zCoord + 0.5f - f, pathpoint2.xCoord + 0.5f + f, pathpoint2.yCoord + 0.25f + 0.01f * j, pathpoint2.zCoord + 0.5f + f).offset(-this.xo, -this.yo, -this.zo), f2, 0.0f, f3, 0.5f);
                        }
                    }
                }
            }
