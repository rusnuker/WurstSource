...
public class DebugRendererNeighborsUpdate implements DebugRenderer.IDebugRenderer
{
    ...
    DebugRendererNeighborsUpdate(final Minecraft p_i47365_1_) {
        this.field_191555_b = Maps.newTreeMap((Comparator)Ordering.natural().reverse());
        ...
        while (iterator.hasNext()) {
            ...
            if (k > 200L) {
                ...
                for (final Map.Entry<BlockPos, Integer> entry2 : map2.entrySet()) {
                    ...
                    if (set.add(blockpos)) {
                        RenderGlobal.drawSelectionBoundingBox(new Box(BlockPos.ORIGIN).expandXyz(0.002).contract(0.0025 * k).offset(blockpos.getX(), blockpos.getY(), blockpos.getZ()).offset(-d0, -d2, -d3), 1.0f, 1.0f, 1.0f, 1.0f);
