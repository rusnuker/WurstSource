    ...
    private PathPoint getSafePoint(final int x, int y, final int z, final int p_186332_4_, final double p_186332_5_, final EnumFacing facing) {
        ...
        if (pathpoint == null && p_186332_4_ > 0 && pathnodetype != PathNodeType.FENCE && pathnodetype != PathNodeType.TRAPDOOR) {
            ...
            if (pathpoint != null && (pathpoint.nodeType == PathNodeType.OPEN || pathpoint.nodeType == PathNodeType.WALKABLE) && this.entity.width < 1.0f) {
                ...
                final Box axisalignedbb = new Box(d3 - d2, y + 0.001, d4 - d2, d3 + d2, y + this.entity.height, d4 + d2);
                final Box axisalignedbb2 = this.blockaccess.getBlockState(blockpos).getBoundingBox(this.blockaccess, blockpos);
                final Box axisalignedbb3 = axisalignedbb.addCoord(0.0, axisalignedbb2.maxY - 0.002, 0.0);
        ...
        if (pathnodetype == PathNodeType.OPEN) {
            final Box axisalignedbb4 = new Box(x - d2 + 0.5, y + 0.001, z - d2 + 0.5, x + d2 + 0.5, y + this.entity.height, z + d2 + 0.5);
