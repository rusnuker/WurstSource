...
public class ServerScoreboard extends Scoreboard
{
    ...
    public ServerScoreboard(final MinecraftServer mcServer) {
        this.addedObjectives = Sets.newHashSet();
    ...
    protected void markSaveDataDirty() {
        Runnable[] dirtyRunnables;
        for (int length = (dirtyRunnables = this.dirtyRunnables).length, i = 0; i < length; ++i) {
            final Runnable runnable = dirtyRunnables[i];
            runnable.run();
        }
    }
    
    public List<Packet<?>> getCreatePackets(final ScoreObjective objective) {
        final List<Packet<?>> list = Lists.newArrayList();
    ...
    public void addObjective(final ScoreObjective objective) {
        ...
        for (final EntityPlayerMP entityplayermp : this.scoreboardMCServer.getPlayerList().getPlayerList()) {
