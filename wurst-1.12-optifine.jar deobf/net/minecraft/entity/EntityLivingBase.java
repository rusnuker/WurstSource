...
{
    private static final Logger field_190632_a;
    ...
    public void onEntityUpdate() {
        ...
        if (this.isEntityAlive()) {
            if (this.isEntityInsideOpaqueBlock()) {
                ...
                if (d0 < 0.0) {
                    ...
                    if (d2 > 0.0) {
                        this.attackEntityFrom(DamageSource.inWall, (float)Math.max(1, MathHelper.floor(-d0 * d2)));
                    }
                }
            }
        }
        ...
        final boolean flag2 = flag && ((EntityPlayer)this).abilities.disableDamage;
        if (this.isEntityAlive()) {
    ...
    public void writeEntityToNBT(final NBTTagCompound compound) {
        ...
        EntityEquipmentSlot[] values2;
        for (int length2 = (values2 = EntityEquipmentSlot.values()).length, j = 0; j < length2; ++j) {
            final EntityEquipmentSlot entityequipmentslot2 = values2[j];
    ...
    public boolean attackEntityFrom(final DamageSource source, float amount) {
        ...
        if (entity2 instanceof EntityPlayerMP) {
            CriteriaTriggers.field_192127_g.func_192220_a((EntityPlayerMP)entity2, this, source, f, amount, flag);
        }
        return flag3;
    }
    ...
    private boolean func_190628_d(final DamageSource p_190628_1_) {
        if (p_190628_1_.canHarmInCreative()) {
            return false;
        }
        ItemStack itemstack = null;
        EnumHand[] values;
        for (int length = (values = EnumHand.values()).length, i = 0; i < length; ++i) {
            final EnumHand enumhand = values[i];
    ...
    private boolean canBlockDamageSource(final DamageSource damageSourceIn) {
        if (!damageSourceIn.isUnblockable() && this.isActiveItemStackBlocking()) {
            ...
            if (vec3d != null) {
                ...
                vec3d3 = new Vec3d(vec3d3.xCoord, 0.0, vec3d3.zCoord);
    ...
    public void renderBrokenItemStack(final ItemStack stack) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            vec3d2 = vec3d2.addVector(this.posX, this.posY + this.getEyeHeight(), this.posZ);
            this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d2.xCoord, vec3d2.yCoord, vec3d2.zCoord, vec3d.xCoord, vec3d.yCoord + 0.05, vec3d.zCoord, Item.getIdFromItem(stack.getItem()));
        }
    }
    ...
    public boolean func_190630_a(final EntityEquipmentSlot p_190630_1_) {
    ...
    public void dismountEntity(final Entity entityIn) {
        if (!(entityIn instanceof EntityBoat) && !(entityIn instanceof AbstractHorse)) {
            ...
            if (enumfacing1 != null) {
                ...
                final Box axisalignedbb = new Box(d4 - d6 / 2.0, entityIn.getEntityBoundingBox().minY, d5 - d7 / 2.0, d4 + d6 / 2.0, Math.floor(entityIn.getEntityBoundingBox().minY) + this.height, d5 + d7 / 2.0);
                int[][] array;
                for (int length = (array = aint1).length, i = 0; i < length; ++i) {
                    final int[] aint2 = array[i];
                    final double d8 = enumfacing1.getFrontOffsetX() * aint2[0] + enumfacing2.getFrontOffsetX() * aint2[1];
                    final double d9 = enumfacing1.getFrontOffsetZ() * aint2[0] + enumfacing2.getFrontOffsetZ() * aint2[1];
                    final double d10 = d4 + d8;
                    final double d11 = d5 + d9;
                    final Box axisalignedbb2 = axisalignedbb.offset(d8, 0.0, d9);
                    if (!this.world.collidesWithAnyBlock(axisalignedbb2)) {
                        if (this.world.getBlockState(new BlockPos(d10, this.posY, d11)).isFullyOpaque()) {
                    ...
                    else if (!this.world.collidesWithAnyBlock(axisalignedbb2.offset(0.0, 1.0, 0.0)) && this.world.getBlockState(new BlockPos(d10, this.posY + 1.0, d11)).isFullyOpaque()) {
                        d1 = d10;
                        d2 = this.posY + 2.0;
                        d3 = d11;
                    }
    ...
    protected void jump() {
        ...
        if (this instanceof EntityPlayer) {
            this.motionY += WurstClient.INSTANCE.hax.highJumpMod.getAdditionalJumpMotion();
        }
    ...
    public void func_191986_a(final float p_191986_1_, final float p_191986_2_, final float p_191986_3_) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            if (!this.isInWater() || WurstClient.INSTANCE.hax.flightMod.isActive() || WurstClient.INSTANCE.hax.freecamMod.isActive() || (this instanceof EntityPlayer && ((EntityPlayer)this).abilities.isFlying)) {
                if (!this.isInLava() || (this instanceof EntityPlayer && ((EntityPlayer)this).abilities.isFlying)) {
                    if (this.isElytraFlying()) {
                        ...
                        final double d6 = Math.sqrt(vec3d.xCoord * vec3d.xCoord + vec3d.zCoord * vec3d.zCoord);
                        final double d7 = Math.sqrt(this.motionX * this.motionX + this.motionZ * this.motionZ);
                        final double d8 = vec3d.lengthVector();
                        ...
                        if (d6 > 0.0) {
                            this.motionX += (vec3d.xCoord / d6 * d7 - this.motionX) * 0.1;
                            this.motionZ += (vec3d.zCoord / d6 * d7 - this.motionZ) * 0.1;
                        }
                        ...
                        if (this.isCollidedHorizontally && !this.world.isRemote) {
                            ...
                            if (f3 > 0.0f) {
                                ...
                                this.attackEntityFrom(DamageSource.flyIntoWall, f3);
                            }
                        }
                        ...
                        this.func_191958_b(p_191986_1_, p_191986_2_, p_191986_3_, f6);
                        f4 = 0.91f;
                        if (this.onGround) {
                    ...
                    if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX, this.motionY + 0.6000000238418579 - this.posY + d13, this.motionZ)) {
                ...
                if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX, this.motionY + 0.6000000238418579 - this.posY + d14, this.motionZ)) {
                    this.motionY = 0.30000001192092896;
                }
            }
        }
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            ...
            EntityEquipmentSlot[] values;
            for (int length = (values = EntityEquipmentSlot.values()).length, j = 0; j < length; ++j) {
                final EntityEquipmentSlot entityequipmentslot = values[j];
                ItemStack itemstack = null;
                ...
                if (!ItemStack.areItemStacksEqual(itemstack2, itemstack)) {
                    ((WorldServer)this.world).getEntityTracker().sendToAllTrackingEntity(this, new SPacketEntityEquipment(this.getEntityId(), entityequipmentslot, itemstack2));
                    if (!itemstack.func_190926_b()) {
                        this.getAttributeMap().removeAttributeModifiers(itemstack.getAttributeModifiers(entityequipmentslot));
                    }
                    if (!itemstack2.func_190926_b()) {
                    ...
                    switch (entityequipmentslot.getSlotType()) {
                        case HAND: {
                            this.handInventory.set(entityequipmentslot.getIndex(), itemstack2.func_190926_b() ? ItemStack.field_190927_a : itemstack2.copy());
                            break;
                        }
                        case ARMOR: {
                            this.armorArray.set(entityequipmentslot.getIndex(), itemstack2.func_190926_b() ? ItemStack.field_190927_a : itemstack2.copy());
                            break;
                        }
                    }
                }
    ...
    public void onLivingUpdate() {
        ...
        this.updateElytra();
        this.func_191986_a(this.moveStrafing, this.notMoveForward, this.moveForward);
        this.world.theProfiler.endSection();
            ...
            if (stack.getItemUseAction() == EnumAction.EAT) {
                for (int i = 0; i < eatingParticleCount; ++i) {
                    ...
                    if (stack.getHasSubtypes()) {
                        this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d2.xCoord, vec3d2.yCoord, vec3d2.zCoord, vec3d.xCoord, vec3d.yCoord + 0.05, vec3d.zCoord, Item.getIdFromItem(stack.getItem()), stack.getMetadata());
                    }
                    else {
                        this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec3d2.xCoord, vec3d2.yCoord, vec3d2.zCoord, vec3d.xCoord, vec3d.yCoord + 0.05, vec3d.zCoord, Item.getIdFromItem(stack.getItem()));
                    }
