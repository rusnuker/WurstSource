...
public class EntityDragon extends EntityLiving implements IEntityMultiPart, IMob
{
        ...
        if (this.getHealth() <= 0.0f) {
            ...
            if (this.isAIDisabled()) {
                ...
                if (this.world.isRemote) {
                    ...
                    if (vec3d != null) {
                        final double d9 = vec3d.xCoord - this.posX;
                        double d10 = vec3d.yCoord - this.posY;
                        final double d11 = vec3d.zCoord - this.posZ;
                        ...
                        final Vec3d vec3d2 = new Vec3d(vec3d.xCoord - this.posX, vec3d.yCoord - this.posY, vec3d.zCoord - this.posZ).normalize();
                        ...
                        this.func_191958_b(0.0f, 0.0f, -1.0f, 0.06f * (f8 * f9 + (1.0f - f9)));
                        if (this.slowed) {
    ...
    private void updateDragonEnderCrystal() {
        ...
        if (this.rand.nextInt(10) == 0) {
            final List<EntityEnderCrystal> list = this.world.getEntitiesWithinAABB((Class<? extends EntityEnderCrystal>)EntityEnderCrystal.class, this.getEntityBoundingBox().expandXyz(32.0));
    ...
    private boolean destroyBlocksInAABB(final Box p_70972_1_) {
    ...
    @Override
    public boolean attackEntityFromPart(final MultiPartEntityPart dragonPart, final DamageSource source, float damage) {
    ...
    public float getHeadPartYOffset(final int p_184667_1_, final double[] p_184667_2_, final double[] p_184667_3_) {
        ...
        final PhaseList<? extends IPhase> phaselist = iphase.getPhaseList();
        double d0;
