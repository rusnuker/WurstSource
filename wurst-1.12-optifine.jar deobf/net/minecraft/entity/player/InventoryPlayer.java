...
public class InventoryPlayer implements IInventory
{
    ...
    public ItemStack getCurrentItem() {
        return isHotbar(this.selectedSlot) ? this.mainInventory.get(this.selectedSlot) : ItemStack.field_190927_a;
    }
    ...
    public void setPickedItemStack(final ItemStack stack) {
        ...
        if (isHotbar(i)) {
            ...
            this.selectedSlot = this.getBestHotbarSlot();
            if (!this.mainInventory.get(this.selectedSlot).func_190926_b()) {
                final int j = this.getFirstEmptyStack();
                if (j != -1) {
                    this.mainInventory.set(j, this.mainInventory.get(this.selectedSlot));
                }
            }
            this.mainInventory.set(this.selectedSlot, stack);
        }
        else {
            this.pickItem(i);
        }
    ...
    public void pickItem(final int index) {
        this.selectedSlot = this.getBestHotbarSlot();
        final ItemStack itemstack = this.mainInventory.get(this.selectedSlot);
        this.mainInventory.set(this.selectedSlot, this.mainInventory.get(index));
    ...
    public int getBestHotbarSlot() {
        for (int i = 0; i < 9; ++i) {
            final int j = (this.selectedSlot + i) % 9;
            if (this.mainInventory.get(j).func_190926_b()) {
                return j;
            }
        }
        for (int k = 0; k < 9; ++k) {
            final int l = (this.selectedSlot + k) % 9;
    ...
    public void changeCurrentItem(int direction) {
        if (WurstClient.INSTANCE.getZoomKey().pressed) {
            return;
        }
        if (direction > 0) {
            direction = 1;
    ...
    public int clearMatchingItems(@Nullable final Item itemIn, final int metadataIn, final int removeCount, @Nullable final NBTTagCompound itemNBT) {
        ...
        for (int j = 0; j < this.getSizeInventory(); ++j) {
            final ItemStack itemstack = this.getInvStack(j);
    ...
    public int storeItemStack(final ItemStack itemStackIn) {
        if (this.canMergeStacks(this.getInvStack(this.selectedSlot), itemStackIn)) {
            return this.selectedSlot;
        }
        if (this.canMergeStacks(this.getInvStack(40), itemStackIn)) {
            return 40;
        }
        for (int i = 0; i < this.mainInventory.size(); ++i) {
        ...
        for (final NonNullList<ItemStack> nonnulllist : this.allInventories) {
            for (int i = 0; i < nonnulllist.size(); ++i) {
                if (!nonnulllist.get(i).func_190926_b()) {
                    nonnulllist.get(i).updateAnimation(this.player.world, this.player, i, this.selectedSlot == i);
                }
            }
        }
    }
    ...
    public boolean func_191971_c(int p_191971_1_, final ItemStack p_191971_2_) {
        ...
        try {
            if (p_191971_2_.isItemDamaged()) {
                ...
                if (p_191971_2_.func_190916_E() == i && this.player.abilities.creativeMode) {
                    p_191971_2_.func_190920_e(0);
                    return true;
                }
    ...
    public void func_191975_a(final World p_191975_1_, final ItemStack p_191975_2_) {
        if (!p_191975_1_.isRemote) {
            while (!p_191975_2_.func_190926_b()) {
                ...
                final int j = p_191975_2_.getMaxStackSize() - this.getInvStack(i).func_190916_E();
                if (!this.func_191971_c(i, p_191975_2_.splitStack(j))) {
                    continue;
                }
                ((EntityPlayerMP)this.player).connection.sendPacket(new SPacketSetSlot(-2, i, this.getInvStack(i)));
            }
        }
    }
    ...
    public float getStrVsBlock(final IBlockState state) {
        float f = 1.0f;
        if (!this.mainInventory.get(this.selectedSlot).func_190926_b()) {
            f *= this.mainInventory.get(this.selectedSlot).getStrVsBlock(state);
        }
        return f;
    }
    ...
    public boolean canHarvestBlock(final IBlockState state) {
        ...
        final ItemStack itemstack = this.getInvStack(this.selectedSlot);
    ...
    public void copyInventory(final InventoryPlayer playerInventory) {
        for (int i = 0; i < this.getSizeInventory(); ++i) {
            this.setInventorySlotContents(i, playerInventory.getInvStack(i));
        }
