...
public class EntityArmorStand extends EntityLivingBase
{
    ...
    private void writePoseToNBT(final NBTTagCompound tagCompound) {
        ...
        this.setBodyRotation(nbttaglist2.hasNoTags() ? EntityArmorStand.DEFAULT_BODY_ROTATION : new Rotations(nbttaglist2));
        final NBTTagList nbttaglist3 = tagCompound.getTagList("LeftArm", 5);
        this.setLeftArmRotation(nbttaglist3.hasNoTags() ? EntityArmorStand.DEFAULT_LEFTARM_ROTATION : new Rotations(nbttaglist3));
        final NBTTagList nbttaglist4 = tagCompound.getTagList("RightArm", 5);
        this.setRightArmRotation(nbttaglist4.hasNoTags() ? EntityArmorStand.DEFAULT_RIGHTARM_ROTATION : new Rotations(nbttaglist4));
        final NBTTagList nbttaglist5 = tagCompound.getTagList("LeftLeg", 5);
        this.setLeftLegRotation(nbttaglist5.hasNoTags() ? EntityArmorStand.DEFAULT_LEFTLEG_ROTATION : new Rotations(nbttaglist5));
        final NBTTagList nbttaglist6 = tagCompound.getTagList("RightLeg", 5);
        this.setRightLegRotation(nbttaglist6.hasNoTags() ? EntityArmorStand.DEFAULT_RIGHTLEG_ROTATION : new Rotations(nbttaglist6));
    }
    ...
    protected void collideWithNearbyEntities() {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (this.getDistanceSqToEntity(entity) <= 0.2) {
    ...
    public EnumActionResult applyPlayerInteraction(final EntityPlayer player, final Vec3d vec, final EnumHand stack) {
        ...
        if (!this.world.isRemote && !player.isSpectator()) {
            ...
            if (itemstack.func_190926_b()) {
                final EntityEquipmentSlot entityequipmentslot2 = this.func_190772_a(vec);
                ...
                if (this.func_190630_a(entityequipmentslot3)) {
                    this.swapItem(player, entityequipmentslot3, itemstack, stack);
                }
            }
            else {
    ...
    protected EntityEquipmentSlot func_190772_a(final Vec3d p_190772_1_) {
        EntityEquipmentSlot entityequipmentslot = EntityEquipmentSlot.MAINHAND;
        final boolean flag = this.isSmall();
        final double d0 = flag ? (p_190772_1_.yCoord * 2.0) : p_190772_1_.yCoord;
        final EntityEquipmentSlot entityequipmentslot2 = EntityEquipmentSlot.FEET;
        if (d0 >= 0.1 && d0 < 0.1 + (flag ? 0.8 : 0.45) && this.func_190630_a(entityequipmentslot2)) {
            entityequipmentslot = EntityEquipmentSlot.FEET;
        }
    ...
    private void swapItem(final EntityPlayer player, final EntityEquipmentSlot p_184795_2_, final ItemStack p_184795_3_, final EnumHand hand) {
        ...
        if ((itemstack.func_190926_b() || (this.disabledSlots & 1 << p_184795_2_.getSlotIndex() + 8) == 0x0) && (!itemstack.func_190926_b() || (this.disabledSlots & 1 << p_184795_2_.getSlotIndex() + 16) == 0x0)) {
            if (player.abilities.creativeMode && itemstack.func_190926_b() && !p_184795_3_.func_190926_b()) {
    ...
    public boolean attackEntityFrom(final DamageSource source, final float amount) {
        ...
        if (source.getEntity() instanceof EntityPlayer && !((EntityPlayer)source.getEntity()).abilities.allowEdit) {
            return false;
        }
