...
public class EntityBoat extends Entity
{
    ...
    protected void entityInit() {
        ...
        DataParameter<Boolean>[] data_ID_PADDLE;
        for (int length = (data_ID_PADDLE = EntityBoat.DATA_ID_PADDLE).length, i = 0; i < length; ++i) {
            final DataParameter<Boolean> dataparameter = data_ID_PADDLE[i];
    ...
    public boolean attackEntityFrom(final DamageSource source, final float amount) {
        ...
        final boolean flag = source.getEntity() instanceof EntityPlayer && ((EntityPlayer)source.getEntity()).abilities.creativeMode;
    ...
    public void onUpdate() {
        ...
        for (int i = 0; i <= 1; ++i) {
            if (this.getPaddleState(i)) {
                if (!this.isSilent() && this.paddlePositions[i] % 6.2831855f <= 0.7853981633974483 && (this.paddlePositions[i] + 0.39269909262657166) % 6.283185307179586 >= 0.7853981633974483) {
                    final SoundEvent soundevent = this.func_193047_k();
                    if (soundevent != null) {
                        final Vec3d vec3d = this.getLook(1.0f);
                        final double d0 = (i == 1) ? (-vec3d.zCoord) : vec3d.zCoord;
                        final double d2 = (i == 1) ? vec3d.xCoord : (-vec3d.xCoord);
        ...
        final List<Entity> list = this.world.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.20000000298023224, -0.009999999776482582, 0.20000000298023224), EntitySelectors.getTeamCollisionPredicate(this));
        if (!list.isEmpty()) {
    ...
    private void tickLerp() {
        if (this.lerpSteps > 0 && !this.canPassengerSteer()) {
            final double d0 = this.posX + (this.boatPitch - this.posX) / this.lerpSteps;
            ...
            final double d4 = MathHelper.wrapDegrees(this.boatYaw - this.rotationYaw);
    ...
    public void setPaddleState(final boolean p_184445_1_, final boolean p_184445_2_) {
        this.dataManager.set(EntityBoat.DATA_ID_PADDLE[0], p_184445_1_);
        this.dataManager.set(EntityBoat.DATA_ID_PADDLE[1], p_184445_2_);
    }
    
    public float getRowingTime(final int p_184448_1_, final float limbSwing) {
        return this.getPaddleState(p_184448_1_) ? ((float)MathHelper.clampedLerp(this.paddlePositions[p_184448_1_] - 0.39269909262657166, this.paddlePositions[p_184448_1_], limbSwing)) : 0.0f;
    }
    ...
    public float getBoatGlide() {
        final Box axisalignedbb = this.getEntityBoundingBox();
        final Box axisalignedbb2 = new Box(axisalignedbb.minX, axisalignedbb.minY - 0.001, axisalignedbb.minZ, axisalignedbb.maxX, axisalignedbb.minY, axisalignedbb.maxZ);
            ...
            for (int k2 = i; k2 < j; ++k2) {
                for (int l2 = k; l2 < l; ++l2) {
                    for (int i3 = i2; i3 < j2; ++i3) {
                        ...
                        final IBlockState iblockstate = this.world.getBlockState(blockpos$pooledmutableblockpos);
                        if (iblockstate.getMaterial() == Material.WATER) {
                            final float f = BlockLiquid.func_190972_g(iblockstate, this.world, blockpos$pooledmutableblockpos);
            ...
            for (int k2 = i; k2 < j; ++k2) {
                for (int l2 = k; l2 < l; ++l2) {
                    for (int i3 = i2; i3 < j2; ++i3) {
                        blockpos$pooledmutableblockpos.setPos(k2, l2, i3);
                        final IBlockState iblockstate = this.world.getBlockState(blockpos$pooledmutableblockpos);
                        if (iblockstate.getMaterial() == Material.WATER && d0 < BlockLiquid.func_190972_g(iblockstate, this.world, blockpos$pooledmutableblockpos)) {
    ...
    public void updatePassenger(final Entity passenger) {
        if (this.isPassenger(passenger)) {
            ...
            passenger.setPosition(this.posX + vec3d.xCoord, this.posY + f2, this.posZ + vec3d.zCoord);
    ...
    public boolean getPaddleState(final int p_184457_1_) {
        return this.dataManager.get(EntityBoat.DATA_ID_PADDLE[p_184457_1_]) && this.getControllingPassenger() != null;
    }
    ...
    public enum Type
    {
        ...
        private Type(final String name, final int ordinal, final int metadataIn, final String nameIn) {
            this.name = nameIn;
