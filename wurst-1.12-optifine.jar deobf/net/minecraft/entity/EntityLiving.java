...
public abstract class EntityLiving extends EntityLivingBase
{
    ...
    public EntityLiving(final World worldIn) {
        ...
        this.mapPathPriority = Maps.newEnumMap((Class)PathNodeType.class);
        this.randomMobsId = 0;
    ...
    protected int getExperiencePoints(final EntityPlayer player) {
        if (this.experienceValue > 0) {
            ...
            for (int j = 0; j < this.inventoryArmor.size(); ++j) {
                if (!this.inventoryArmor.get(j).func_190926_b() && this.inventoryArmorDropChances[j] <= 1.0f) {
                    i += 1 + this.rand.nextInt(3);
                }
            }
            for (int k = 0; k < this.inventoryHands.size(); ++k) {
                if (!this.inventoryHands.get(k).func_190926_b() && this.inventoryHandsDropChances[k] <= 1.0f) {
                    i += 1 + this.rand.nextInt(3);
                }
            }
            return i;
    ...
    public void writeEntityToNBT(final NBTTagCompound compound) {
        ...
        float[] inventoryArmorDropChances;
        for (int length = (inventoryArmorDropChances = this.inventoryArmorDropChances).length, i = 0; i < length; ++i) {
            final float f = inventoryArmorDropChances[i];
        ...
        float[] inventoryHandsDropChances;
        for (int length2 = (inventoryHandsDropChances = this.inventoryHandsDropChances).length, j = 0; j < length2; ++j) {
            final float f2 = inventoryHandsDropChances[j];
        ...
        if (this.leashedToEntity != null) {
            ...
            if (this.leashedToEntity instanceof EntityLivingBase) {
                final UUID uuid = this.leashedToEntity.getUniqueID();
    ...
    public void func_191989_p(final float p_191989_1_) {
    ...
    public void onLivingUpdate() {
        ...
        if (!this.world.isRemote && this.canPickUpLoot() && !this.dead && this.world.getGameRules().getBoolean("mobGriefing")) {
            for (final EntityItem entityitem : this.world.getEntitiesWithinAABB((Class<? extends EntityItem>)EntityItem.class, this.getEntityBoundingBox().expand(1.0, 0.0, 1.0))) {
    ...
    protected void updateEquipmentIfNeeded(final EntityItem itemEntity) {
        ...
        if (!itemstack2.func_190926_b()) {
            ...
            else if (itemstack.getItem() instanceof ItemArmor && itemstack2.getItem() instanceof ItemArmor && !EnchantmentHelper.func_190938_b(itemstack2)) {
        ...
        if (flag && this.canEquipItem(itemstack)) {
            ...
            if (!itemstack2.func_190926_b() && this.rand.nextFloat() - 0.1f < d0) {
    ...
    @Override
    public final boolean processInitialInteract(final EntityPlayer player, final EnumHand stack) {
        if (this.getLeashed() && this.getLeashedToEntity() == player) {
            this.clearLeashed(true, !player.abilities.creativeMode);
            return true;
        }
        ...
        return this.processInteract(player, stack) || super.processInitialInteract(player, stack);
    }
    ...
    private void recreateLeash() {
        if (this.isLeashed && this.leashNBTTag != null) {
            if (this.leashNBTTag.hasUniqueId("UUID")) {
                ...
                for (final EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB((Class<? extends EntityLivingBase>)EntityLivingBase.class, this.getEntityBoundingBox().expandXyz(10.0))) {
    ...
    public void setLeftHanded(final boolean disable) {
        final byte b0 = this.dataManager.get(EntityLiving.AI_FLAGS);
        this.dataManager.set(EntityLiving.AI_FLAGS, disable ? ((byte)(b0 | 0x2)) : ((byte)(b0 & 0xFFFFFFFD)));
    }
