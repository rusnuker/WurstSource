...
public abstract class MinecraftServer implements ICommandSender, Runnable, IThreadListener, ISnooperInfo
{
    ...
    public void setResourcePackFromWorld(final String worldNameIn, final ISaveHandler saveHandlerIn) {
        ...
        if (file1.isFile()) {
            try {
                ...
                MinecraftServer.LOG.warn("Something went wrong url encoding {}", (Object)worldNameIn);
            }
        }
    }
    ...
    protected void outputPercentRemaining(final String message, final int percent) {
        ...
        MinecraftServer.LOG.info("{}: {}%", (Object)message, (Object)percent);
    }
    ...
    public void saveAllWorlds(final boolean isSilent) {
        ...
        for (int length = (worldServers = this.worldServers).length, i = 0; i < length; ++i) {
            ...
            if (worldserver != null) {
                if (!isSilent) {
                    MinecraftServer.LOG.info("Saving chunks for level '{}'/{}", (Object)worldserver.getWorldInfo().getWorldName(), (Object)worldserver.provider.getDimensionType().getName());
                }
                try {
    ...
    public void applyServerIconToResponse(final ServerStatusResponse response) {
        ...
        if (file1.isFile()) {
            ...
            try {
                ...
                MinecraftServer.LOG.error("Couldn't load server icon", (Throwable)exception);
                return;
            }
            finally {
                bytebuf.release();
    ...
    public void addServerStatsToSnooper(final Snooper playerSnooper) {
        ...
        if (this.worldServers != null) {
            WorldServer[] worldServers;
            for (int length = (worldServers = this.worldServers).length, j = 0; j < length; ++j) {
                final WorldServer worldserver = worldServers[j];
                if (worldserver != null) {
