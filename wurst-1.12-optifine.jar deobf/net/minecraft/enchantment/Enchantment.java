...
public abstract class Enchantment
{
    ...
    public List<ItemStack> getEntityEquipment(final EntityLivingBase entityIn) {
        ...
        EntityEquipmentSlot[] applicableEquipmentTypes;
        for (int length = (applicableEquipmentTypes = this.applicableEquipmentTypes).length, i = 0; i < length; ++i) {
            final EntityEquipmentSlot entityequipmentslot = applicableEquipmentTypes[i];
    ...
    public final boolean func_191560_c(final Enchantment p_191560_1_) {
    ...
    public String getTranslatedName(final int level) {
        ...
        return (level == 1 && this.getMaxLevel() == 1) ? s : (String.valueOf(s) + " " + I18n.translateToLocal("enchantment.level." + level));
    }
    ...
    public enum Rarity
    {
        ...
        private Rarity(final String name, final int ordinal, final int rarityWeight) {
