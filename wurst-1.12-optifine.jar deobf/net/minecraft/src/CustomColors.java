...
public class CustomColors
{
    ...
    public static void update() {
        ...
        if (Config.isCustomColors()) {
            ...
            CustomColors.lightMapsColorsRgb = (CustomColormap[])pair.getLeft();
    ...
    private static Pair<CustomColormap[], Integer> parseLightmapsRgb() {
        ...
        return (Pair<CustomColormap[], Integer>)new ImmutablePair((Object)acustomcolormap, (Object)j2);
    }
    ...
    private static CustomColormap[] readCustomColormaps(final Properties p_readCustomColormaps_0_, final String p_readCustomColormaps_1_) {
        ...
        for (final Object s2 : p_readCustomColormaps_0_.keySet()) {
            ...
            final String s4 = p_readCustomColormaps_0_.getProperty(s3);
            if (s3.startsWith(s)) {
                map.put(s3, s4);
            }
        }
        ...
        for (int j = 0; j < astring.length; ++j) {
            final String s5 = astring[j];
            final String s6 = p_readCustomColormaps_0_.getProperty(s5);
            dbg("Block palette: " + s5 + " = " + s6);
            String s7 = s5.substring(s.length());
            final String s8 = TextureUtils.getBasePath(p_readCustomColormaps_1_);
            s7 = TextureUtils.fixResourcePath(s7, s8);
            final CustomColormap customcolormap = getCustomColors(s7, 256, 256);
            if (customcolormap == null) {
                warn("Colormap not found: " + s7);
            }
            else {
                final ConnectedParser connectedparser = new ConnectedParser("CustomColors");
                final MatchBlock[] amatchblock = connectedparser.parseMatchBlocks(s6);
    ...
    public static Vec3d getUnderwaterColor(final IBlockAccess p_getUnderwaterColor_0_, final double p_getUnderwaterColor_1_, final double p_getUnderwaterColor_3_, final double p_getUnderwaterColor_5_) {
        ...
        final float f2 = k / 255.0f;
        final float f3 = l / 255.0f;
        final Vec3d vec3d = CustomColors.underwaterColorFader.getColor(f, f2, f3);
        return vec3d;
    }
    ...
    private static int[] readSpawnEggColors(final Properties p_readSpawnEggColors_0_, final String p_readSpawnEggColors_1_, final String p_readSpawnEggColors_2_, final String p_readSpawnEggColors_3_) {
        ...
        int i = 0;
        for (final Object s0 : set) {
            final String s2 = (String)s0;
            final String s3 = p_readSpawnEggColors_0_.getProperty(s2);
            if (s2.startsWith(p_readSpawnEggColors_2_)) {
                final String s4 = StrUtils.removePrefix(s2, p_readSpawnEggColors_2_);
                int j = EntityUtils.getEntityIdByName(s4);
                if (j < 0) {
                    j = EntityUtils.getEntityIdByLocation(new ResourceLocation(s4).toString());
                }
                if (j < 0) {
                    warn("Invalid spawn egg name: " + s2);
                }
                else {
                    final int k = parseColor(s3);
                    if (k < 0) {
                        warn("Invalid spawn egg color: " + s2 + " = " + s3);
                    }
                    else {
                        while (list.size() <= j) {
                            list.add(-1);
    ...
    private static float[][] readDyeColors(final Properties p_readDyeColors_0_, final String p_readDyeColors_1_, final String p_readDyeColors_2_, final String p_readDyeColors_3_) {
        ...
        int k = 0;
        for (final Object s0 : p_readDyeColors_0_.keySet()) {
            final String s2 = (String)s0;
            final String s3 = p_readDyeColors_0_.getProperty(s2);
            if (s2.startsWith(p_readDyeColors_2_)) {
                String s4 = StrUtils.removePrefix(s2, p_readDyeColors_2_);
                if (s4.equals("lightBlue")) {
                    s4 = "light_blue";
                }
                final EnumDyeColor enumdyecolor2 = map.get(s4);
                final int j = parseColor(s3);
    ...
    private static int[] readTextColors(final Properties p_readTextColors_0_, final String p_readTextColors_1_, final String p_readTextColors_2_, final String p_readTextColors_3_) {
        ...
        Arrays.fill(aint, -1);
        int i = 0;
        for (final Object s0 : p_readTextColors_0_.keySet()) {
            final String s2 = (String)s0;
            final String s3 = p_readTextColors_0_.getProperty(s2);
            if (s2.startsWith(p_readTextColors_2_)) {
                final String s4 = StrUtils.removePrefix(s2, p_readTextColors_2_);
    ...
    private static int[] readMapColors(final Properties p_readMapColors_0_, final String p_readMapColors_1_, final String p_readMapColors_2_, final String p_readMapColors_3_) {
        ...
        Arrays.fill(aint, -1);
        int i = 0;
        for (final Object s0 : p_readMapColors_0_.keySet()) {
            final String s2 = (String)s0;
            final String s3 = p_readMapColors_0_.getProperty(s2);
            if (s2.startsWith(p_readMapColors_2_)) {
                final String s4 = StrUtils.removePrefix(s2, p_readMapColors_2_);
                final int j = getMapColorIndex(s4);
                final int k = parseColor(s3);
                if (j >= 0 && j < aint.length && k >= 0) {
                    aint[j] = k;
                    ++i;
    ...
    private static int[] readPotionColors(final Properties p_readPotionColors_0_, final String p_readPotionColors_1_, final String p_readPotionColors_2_, final String p_readPotionColors_3_) {
        ...
        Arrays.fill(aint, -1);
        int i = 0;
        for (final Object s0 : p_readPotionColors_0_.keySet()) {
            final String s2 = (String)s0;
            final String s3 = p_readPotionColors_0_.getProperty(s2);
            if (s2.startsWith(p_readPotionColors_2_)) {
                final int j = getPotionId(s2);
                final int k = parseColor(s3);
                if (j >= 0 && j < aint.length && k >= 0) {
                    aint[j] = k;
                    ++i;
