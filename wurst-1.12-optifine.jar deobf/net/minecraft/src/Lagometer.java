...
public class Lagometer
{
    ...
    public static void showLagometer(final ScaledResolution p_showLagometer_0_) {
        if (Lagometer.gameSettings != null && (Lagometer.gameSettings.ofLagometer || Lagometer.gameSettings.showLagometer)) {
            ...
            for (int j = 0; j < Lagometer.timesFrame.length; ++j) {
                ...
                final long l = 0L;
                if (Lagometer.gcs[j]) {
                    renderTime(j, Lagometer.timesFrame[j], k, k / 2, 0, f, bufferbuilder);
                }
                else {
                    renderTime(j, Lagometer.timesFrame[j], k, k, k, f, bufferbuilder);
                    f -= renderTime(j, Lagometer.timesServer[j], k / 2, k / 2, k / 2, f, bufferbuilder);
                    f -= renderTime(j, Lagometer.timesTerrain[j], 0, k, 0, f, bufferbuilder);
                    f -= renderTime(j, Lagometer.timesVisibility[j], k, k, 0, f, bufferbuilder);
                    f -= renderTime(j, Lagometer.timesChunkUpdate[j], k, 0, 0, f, bufferbuilder);
                    f -= renderTime(j, Lagometer.timesChunkUpload[j], k, 0, k, f, bufferbuilder);
                    f -= renderTime(j, Lagometer.timesScheduledExecutables[j], 0, 0, k, f, bufferbuilder);
                    final float n = f - renderTime(j, Lagometer.timesTick[j], 0, k, k, f, bufferbuilder);
                }
            }
            renderTimeDivider(0, Lagometer.timesFrame.length, 33333333L, 196, 196, 196, (float)Lagometer.mc.displayHeight, bufferbuilder);
            renderTimeDivider(0, Lagometer.timesFrame.length, 16666666L, 196, 196, 196, (float)Lagometer.mc.displayHeight, bufferbuilder);
            tessellator.draw();
            ...
            final int i3 = Lagometer.mc.displayHeight / p_showLagometer_0_.getScaleFactor() - 8;
    ...
    private static long renderTime(final int p_renderTime_0_, final long p_renderTime_1_, final int p_renderTime_3_, final int p_renderTime_4_, final int p_renderTime_5_, final float p_renderTime_6_, final BufferBuilder p_renderTime_7_) {
    ...
    private static long renderTimeDivider(final int p_renderTimeDivider_0_, final int p_renderTimeDivider_1_, final long p_renderTimeDivider_2_, final int p_renderTimeDivider_4_, final int p_renderTimeDivider_5_, final int p_renderTimeDivider_6_, final float p_renderTimeDivider_7_, final BufferBuilder p_renderTimeDivider_8_) {
