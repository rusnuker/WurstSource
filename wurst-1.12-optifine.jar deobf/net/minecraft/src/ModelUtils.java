...
public class ModelUtils
{
    public static void dbgModel(final IBakedModel p_dbgModel_0_) {
        if (p_dbgModel_0_ != null) {
            ...
            for (int i = 0; i < aenumfacing.length; ++i) {
                ...
                final List list = p_dbgModel_0_.getQuads(null, enumfacing, 0L);
                dbgQuads(enumfacing.getName(), list, "  ");
            }
            final List list2 = p_dbgModel_0_.getQuads(null, null, 0L);
            dbgQuads("General", list2, "  ");
        }
    }
    
    private static void dbgQuads(final String p_dbgQuads_0_, final List<BakedQuad> p_dbgQuads_1_, final String p_dbgQuads_2_) {
        for (final BakedQuad bakedquad : p_dbgQuads_1_) {
    ...
    public static IBakedModel duplicateModel(final IBakedModel p_duplicateModel_0_) {
        final List list = duplicateQuadList(p_duplicateModel_0_.getQuads(null, null, 0L));
        final EnumFacing[] aenumfacing = EnumFacing.VALUES;
        final Map<EnumFacing, List<BakedQuad>> map = new HashMap<EnumFacing, List<BakedQuad>>();
        for (int i = 0; i < aenumfacing.length; ++i) {
            final EnumFacing enumfacing = aenumfacing[i];
            final List list2 = p_duplicateModel_0_.getQuads(null, enumfacing, 0L);
            final List list3 = duplicateQuadList(list2);
            map.put(enumfacing, list3);
        }
        final SimpleBakedModel simplebakedmodel = new SimpleBakedModel(list, map, p_duplicateModel_0_.isAmbientOcclusion(), p_duplicateModel_0_.isGui3d(), p_duplicateModel_0_.getParticleTexture(), p_duplicateModel_0_.getItemCameraTransforms(), p_duplicateModel_0_.getOverrides());
        return simplebakedmodel;
    }
    
    public static List duplicateQuadList(final List<BakedQuad> p_duplicateQuadList_0_) {
        final List list = new ArrayList();
        for (final BakedQuad bakedquad : p_duplicateQuadList_0_) {
