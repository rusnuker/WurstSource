...
public class CustomSkyLayer
{
    ...
    public CustomSkyLayer(final Properties p_i32_1_, final String p_i32_2_) {
        this.source = null;
        this.startFadeIn = -1;
        this.endFadeIn = -1;
        this.startFadeOut = -1;
        ...
        this.startFadeIn = this.parseTime(p_i32_1_.getProperty("startFadeIn"));
        this.endFadeIn = this.parseTime(p_i32_1_.getProperty("endFadeIn"));
        this.startFadeOut = this.parseTime(p_i32_1_.getProperty("startFadeOut"));
        this.endFadeOut = this.parseTime(p_i32_1_.getProperty("endFadeOut"));
        this.blend = Blender.parseBlend(p_i32_1_.getProperty("blend"));
        this.rotate = this.parseBoolean(p_i32_1_.getProperty("rotate"), true);
        this.speed = this.parseFloat(p_i32_1_.getProperty("speed"), 1.0f);
        this.axis = this.parseAxis(p_i32_1_.getProperty("axis"), CustomSkyLayer.DEFAULT_AXIS);
        ...
        final List<String> list = this.parseWeatherList(p_i32_1_.getProperty("weather", "clear"));
        this.weatherClear = list.contains("clear");
        this.weatherRain = list.contains("rain");
        this.weatherThunder = list.contains("thunder");
    }
    
    private List<String> parseWeatherList(final String p_parseWeatherList_1_) {
        final List<String> list = Arrays.asList("clear", "rain", "thunder");
        final List<String> list2 = new ArrayList<String>();
        final String[] astring = Config.tokenize(p_parseWeatherList_1_, " ");
    ...
    public void render(final int p_render_1_, final float p_render_2_, final float p_render_3_, final float p_render_4_) {
        final float f = 1.0f - p_render_3_;
        final float f2 = p_render_3_ - p_render_4_;
        float f3 = 0.0f;
        ...
        float f4 = f3 * this.getFadeBrightness(p_render_1_);
    ...
    private void renderSide(final Tessellator p_renderSide_1_, final int p_renderSide_2_) {
        final BufferBuilder bufferbuilder = p_renderSide_1_.getBuffer();
        final double d0 = p_renderSide_2_ % 3 / 3.0;
        final double d2 = p_renderSide_2_ / 3 / 2.0;
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
        bufferbuilder.pos(-100.0, -100.0, -100.0).tex(d0, d2).endVertex();
        bufferbuilder.pos(-100.0, -100.0, 100.0).tex(d0, d2 + 0.5).endVertex();
        bufferbuilder.pos(100.0, -100.0, 100.0).tex(d0 + 0.3333333333333333, d2 + 0.5).endVertex();
        bufferbuilder.pos(100.0, -100.0, -100.0).tex(d0 + 0.3333333333333333, d2).endVertex();
