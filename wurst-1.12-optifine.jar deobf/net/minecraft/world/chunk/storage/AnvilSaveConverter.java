...
public class AnvilSaveConverter extends SaveFormatOld
{
    ...
    public List<WorldSummary> getSaveList() throws AnvilConverterException {
        if (this.savesDirectory != null && this.savesDirectory.exists() && this.savesDirectory.isDirectory()) {
            final List<WorldSummary> list = Lists.newArrayList();
    ...
    public boolean convertMapFormat(final String filename, final IProgressUpdate progressCallback) {
        ...
        final List<File> list2 = Lists.newArrayList();
        final List<File> list3 = Lists.newArrayList();
    ...
    private void convertChunks(final File baseFolder, final File p_75811_2_, final BiomeProvider biomeSource, final int p_75811_4_, final int p_75811_5_, final IProgressUpdate progressCallback) {
        try {
            ...
            final RegionFile regionfile2 = new RegionFile(new File(baseFolder, String.valueOf(s.substring(0, s.length() - ".mcr".length())) + ".mca"));
            for (int i = 0; i < 32; ++i) {
                for (int j = 0; j < 32; ++j) {
