...
public class Template
{
    ...
    public void takeBlocksFromWorld(final World worldIn, final BlockPos startPos, final BlockPos endPos, final boolean takeEntities, @Nullable final Block toIgnore) {
        if (endPos.getX() >= 1 && endPos.getY() >= 1 && endPos.getZ() >= 1) {
            final BlockPos blockpos = startPos.add(endPos).add(-1, -1, -1);
            final List<BlockInfo> list = Lists.newArrayList();
            final List<BlockInfo> list2 = Lists.newArrayList();
            final List<BlockInfo> list3 = Lists.newArrayList();
    ...
    private void takeEntitiesFromWorld(final World worldIn, final BlockPos startPos, final BlockPos endPos) {
        final List<Entity> list = worldIn.getEntitiesWithinAABB((Class<? extends Entity>)Entity.class, new Box(startPos, endPos), (com.google.common.base.Predicate<? super Entity>)new Predicate<Entity>() {
    ...
    public Map<BlockPos, String> getDataBlocks(final BlockPos pos, final PlacementSettings placementIn) {
        final Map<BlockPos, String> map = Maps.newHashMap();
    ...
    public static BlockPos transformedBlockPos(final PlacementSettings placementIn, final BlockPos p_186266_1_) {
    ...
    public void addBlocksToWorld(final World p_189960_1_, final BlockPos p_189960_2_, @Nullable final ITemplateProcessor p_189960_3_, final PlacementSettings p_189960_4_, final int p_189960_5_) {
        if ((!this.blocks.isEmpty() || (!p_189960_4_.getIgnoreEntities() && !this.entities.isEmpty())) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
            final Block block = p_189960_4_.getReplacedBlock();
            final StructureBoundingBox structureboundingbox = p_189960_4_.getBoundingBox();
            for (final BlockInfo template$blockinfo : this.blocks) {
                final BlockPos blockpos = transformedBlockPos(p_189960_4_, template$blockinfo.pos).add(p_189960_2_);
                final BlockInfo template$blockinfo2 = (p_189960_3_ != null) ? p_189960_3_.processBlock(p_189960_1_, blockpos, template$blockinfo) : template$blockinfo;
                if (template$blockinfo2 != null) {
                    final Block block2 = template$blockinfo2.blockState.getBlock();
                    if ((block != null && block == block2) || (p_189960_4_.getIgnoreStructureBlock() && block2 == Blocks.STRUCTURE_BLOCK) || (structureboundingbox != null && !structureboundingbox.isVecInside(blockpos))) {
                        continue;
                    }
                    final IBlockState iblockstate = template$blockinfo2.blockState.withMirror(p_189960_4_.getMirror());
                    final IBlockState iblockstate2 = iblockstate.withRotation(p_189960_4_.getRotation());
                    if (template$blockinfo2.tileentityData != null) {
                        final TileEntity tileentity = p_189960_1_.getTileEntity(blockpos);
                        if (tileentity != null) {
                    ...
                    if (!p_189960_1_.setBlockState(blockpos, iblockstate2, p_189960_5_) || template$blockinfo2.tileentityData == null) {
                        continue;
                    }
                    final TileEntity tileentity2 = p_189960_1_.getTileEntity(blockpos);
                    if (tileentity2 == null) {
                        continue;
                    }
            ...
            for (final BlockInfo template$blockinfo3 : this.blocks) {
                if (block == null || block != template$blockinfo3.blockState.getBlock()) {
                    final BlockPos blockpos2 = transformedBlockPos(p_189960_4_, template$blockinfo3.pos).add(p_189960_2_);
                    if (structureboundingbox != null && !structureboundingbox.isVecInside(blockpos2)) {
                        continue;
                    }
                    p_189960_1_.notifyNeighborsRespectDebug(blockpos2, template$blockinfo3.blockState.getBlock(), false);
                    if (template$blockinfo3.tileentityData == null) {
                        continue;
                    }
                    final TileEntity tileentity3 = p_189960_1_.getTileEntity(blockpos2);
                    if (tileentity3 == null) {
                        continue;
                    }
    ...
    private void addEntitiesToWorld(final World worldIn, final BlockPos pos, final Mirror mirrorIn, final Rotation rotationIn, @Nullable final StructureBoundingBox aabb) {
        for (final EntityInfo template$entityinfo : this.entities) {
            ...
            if (aabb == null || aabb.isVecInside(blockpos)) {
                ...
                final Vec3d vec3d2 = vec3d.addVector(pos.getX(), pos.getY(), pos.getZ());
                final NBTTagList nbttaglist = new NBTTagList();
                nbttaglist.appendTag(new NBTTagDouble(vec3d2.xCoord));
                nbttaglist.appendTag(new NBTTagDouble(vec3d2.yCoord));
                nbttaglist.appendTag(new NBTTagDouble(vec3d2.zCoord));
                ...
                entity.setLocationAndAngles(vec3d2.xCoord, vec3d2.yCoord, vec3d2.zCoord, f, entity.rotationPitch);
    ...
    public static BlockPos func_191157_a(final BlockPos p_191157_0_, final Mirror p_191157_1_, final Rotation p_191157_2_, int p_191157_3_, int p_191157_4_) {
        --p_191157_3_;
        --p_191157_4_;
    ...
    public static void func_191158_a(final DataFixer p_191158_0_) {
    ...
    public NBTTagCompound writeToNBT(final NBTTagCompound nbt) {
        final BasicPalette template$basicpalette = new BasicPalette(null);
        final NBTTagList nbttaglist = new NBTTagList();
        for (final BlockInfo template$blockinfo : this.blocks) {
            final NBTTagCompound nbttagcompound = new NBTTagCompound();
        ...
        for (final EntityInfo template$entityinfo : this.entities) {
            ...
            nbttagcompound2.setTag("pos", this.writeDoubles(template$entityinfo.pos.xCoord, template$entityinfo.pos.yCoord, template$entityinfo.pos.zCoord));
    ...
    static class BasicPalette implements Iterable<IBlockState>
    {
        ...
        public int idFor(final IBlockState p_189954_1_) {
        ...
        @Nullable
        public IBlockState stateFor(final int p_189955_1_) {
            final IBlockState iblockstate = this.ids.getByValue(p_189955_1_);
