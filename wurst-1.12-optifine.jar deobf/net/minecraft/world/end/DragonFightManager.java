...
public class DragonFightManager
{
    ...
    protected void setRespawnState(final DragonSpawnManager state) {
        ...
        if (state == DragonSpawnManager.END) {
            this.respawnState = null;
            this.dragonKilled = false;
            final EntityDragon entitydragon = this.func_192445_m();
    ...
    private boolean hasDragonBeenKilled() {
        for (int i = -8; i <= 8; ++i) {
            for (int j = -8; j <= 8; ++j) {
                final Chunk chunk = this.world.getChunkFromChunkCoords(i, j);
    ...
    private void updateplayers() {
        final Set<EntityPlayerMP> set = Sets.newHashSet();
        ...
        final Set<EntityPlayerMP> set2 = Sets.newHashSet((Iterable)this.bossInfo.getPlayers());
        set2.removeAll(set);
    ...
    private void findAliveCrystals() {
        ...
        WorldGenSpikes.EndSpike[] spikesForWorld;
        for (int length = (spikesForWorld = BiomeEndDecorator.getSpikesForWorld(this.world)).length, i = 0; i < length; ++i) {
            final WorldGenSpikes.EndSpike worldgenspikes$endspike = spikesForWorld[i];
    ...
    public void respawnDragon() {
        if (this.dragonKilled && this.respawnState == null) {
            ...
            final List<EntityEnderCrystal> list1 = Lists.newArrayList();
            final BlockPos blockpos2 = blockpos.up(1);
            for (final EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
                final List<EntityEnderCrystal> list2 = this.world.getEntitiesWithinAABB((Class<? extends EntityEnderCrystal>)EntityEnderCrystal.class, new Box(blockpos2.offset(enumfacing, 2)));
                if (list2.isEmpty()) {
                    return;
                }
                list1.addAll(list2);
