...
public abstract class World implements IBlockAccess
{
    ...
    public boolean func_190526_b(final int p_190526_1_, final int p_190526_2_) {
        return this.isChunkLoaded(p_190526_1_, p_190526_2_, false) || this.chunkProvider.func_191062_e(p_190526_1_, p_190526_2_);
    }
    ...
    public void func_190522_c(final BlockPos p_190522_1_, final Block p_190522_2_) {
    ...
    public void notifyNeighborsOfStateChange(final BlockPos pos, final Block blockType, final boolean p_175685_3_) {
    ...
    public void func_190524_a(final BlockPos p_190524_1_, final Block p_190524_2_, final BlockPos p_190524_3_) {
        if (!this.isRemote) {
            final IBlockState iblockstate = this.getBlockState(p_190524_1_);
            try {
    ...
    public void func_190529_b(final BlockPos p_190529_1_, final Block p_190529_2_, final BlockPos p_190529_3_) {
        if (!this.isRemote) {
            final IBlockState iblockstate = this.getBlockState(p_190529_1_);
    ...
    public int getHeight(final int x, final int z) {
        ...
        if (x >= -30000000 && z >= -30000000 && x < 30000000 && z < 30000000) {
            if (this.isChunkLoaded(x >> 4, z >> 4, true)) {
                i = this.getChunkFromChunkCoords(x >> 4, z >> 4).getHeightValue(x & 0xF, z & 0xF);
            }
            else {
                i = 0;
            }
    ...
    public RayTraceResult rayTraceBlocks(Vec3d vec31, final Vec3d vec32, final boolean stopOnLiquid, final boolean ignoreBlockWithoutBoundingBox, final boolean returnLastUncollidableBlock) {
        ...
        if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
            final int i = MathHelper.floor(vec32.xCoord);
            final int j = MathHelper.floor(vec32.yCoord);
            final int k = MathHelper.floor(vec32.zCoord);
            int l = MathHelper.floor(vec31.xCoord);
            int i2 = MathHelper.floor(vec31.yCoord);
            int j2 = MathHelper.floor(vec31.zCoord);
            ...
            while (k2-- >= 0) {
                ...
                i2 = MathHelper.floor(vec31.yCoord) - ((enumfacing == EnumFacing.UP) ? 1 : 0);
                j2 = MathHelper.floor(vec31.zCoord) - ((enumfacing == EnumFacing.SOUTH) ? 1 : 0);
    ...
    public void func_190523_a(final int p_190523_1_, final double p_190523_2_, final double p_190523_4_, final double p_190523_6_, final double p_190523_8_, final double p_190523_10_, final double p_190523_12_, final int... p_190523_14_) {
        for (int i = 0; i < this.eventListeners.size(); ++i) {
    ...
    private boolean func_191504_a(@Nullable final Entity p_191504_1_, final Box p_191504_2_, final boolean p_191504_3_, @Nullable final List<Box> p_191504_4_) {
        final int i = MathHelper.floor(p_191504_2_.minX) - 1;
        final int j = MathHelper.ceil(p_191504_2_.maxX) + 1;
        final int k = MathHelper.floor(p_191504_2_.minY) - 1;
        final int l = MathHelper.ceil(p_191504_2_.maxY) + 1;
        final int i2 = MathHelper.floor(p_191504_2_.minZ) - 1;
        final int j2 = MathHelper.ceil(p_191504_2_.maxZ) + 1;
        final WorldBorder worldborder = this.getWorldBorder();
        final boolean flag = p_191504_1_ != null && p_191504_1_.isOutsideBorder();
        final boolean flag2 = p_191504_1_ != null && this.func_191503_g(p_191504_1_);
                    ...
                    if ((!flag3 || !flag4) && this.isBlockLoaded(blockpos$pooledmutableblockpos.setPos(k2, 64, l2))) {
                        for (int i3 = k; i3 < l; ++i3) {
                            if ((!flag3 && !flag4) || i3 != l - 1) {
                                ...
                                else if (p_191504_1_ != null && flag == flag2) {
                                    p_191504_1_.setOutsideBorder(!flag2);
                                }
                                ...
                                iblockstate2.addCollisionBoxToList(this, blockpos$pooledmutableblockpos, p_191504_2_, p_191504_4_, p_191504_1_, false);
    ...
    public List<Box> getCollisionBoxes(@Nullable final Entity entityIn, final Box aabb) {
        final List<Box> list = Lists.newArrayList();
        this.func_191504_a(entityIn, aabb, false, list);
        if (entityIn != null) {
            final List<Entity> list2 = this.getEntitiesWithinAABBExcludingEntity(entityIn, aabb.expandXyz(0.25));
            for (int i = 0; i < list2.size(); ++i) {
                final Entity entity = list2.get(i);
                if (!entityIn.isRidingSameEntity(entity)) {
                    Box axisalignedbb = entity.getCollisionBoundingBox();
    ...
    public boolean func_191503_g(final Entity p_191503_1_) {
        ...
        return p_191503_1_.posX > d0 && p_191503_1_.posX < d3 && p_191503_1_.posZ > d2 && p_191503_1_.posZ < d4;
    }
    
    public boolean collidesWithAnyBlock(final Box bbox) {
    ...
    public Vec3d getSkyColor(final Entity entityIn, final float partialTicks) {
        ...
        final float f3 = biome.getFloatTemperature(blockpos);
    ...
    public float getCelestialAngle(final float partialTicks) {
        if (WurstClient.INSTANCE.hax.noWeatherMod.isTimeChanged()) {
            return this.provider.calculateCelestialAngle(WurstClient.INSTANCE.hax.noWeatherMod.getChangedTime(), partialTicks);
        }
        return this.provider.calculateCelestialAngle(this.worldInfo.getWorldTime(), partialTicks);
    }
    
    public int getMoonPhase() {
        if (WurstClient.INSTANCE.hax.noWeatherMod.isMoonPhaseChanged()) {
            return WurstClient.INSTANCE.hax.noWeatherMod.getChangedMoonPhase();
        }
    ...
    public void updateEntities() {
        ...
        for (int k = 0; k < this.unloadedEntityList.size(); ++k) {
            ...
            if (entity2.addedToChunk && this.isChunkLoaded(j, k2, true)) {
                this.getChunkFromChunkCoords(j, k2).removeEntity(entity2);
            }
        }
        ...
        for (int i2 = 0; i2 < this.loadedEntityList.size(); ++i2) {
            ...
            if (entity3.isDead) {
                ...
                if (entity3.addedToChunk && this.isChunkLoaded(l2, i3, true)) {
                    this.getChunkFromChunkCoords(l2, i3).removeEntity(entity3);
                }
        ...
        while (iterator.hasNext()) {
            ...
            if (!tileentity.isInvalid() && tileentity.hasWorldObj()) {
                ...
                if (this.isBlockLoaded(blockpos) && this.worldBorder.contains(blockpos)) {
                    ...
                    catch (final Throwable throwable4) {
            ...
            for (int j2 = 0; j2 < this.addedTileEntityList.size(); ++j2) {
                ...
                if (!tileentity2.isInvalid()) {
                    ...
                    if (this.isBlockLoaded(tileentity2.getPos())) {
                        final Chunk chunk = this.getChunkFromBlockCoords(tileentity2.getPos());
    ...
    public void updateEntityWithOptionalForce(final Entity entityIn, final boolean forceUpdate) {
        if (!(entityIn instanceof EntityPlayer)) {
            final int i = MathHelper.floor(entityIn.posX);
            final int j = MathHelper.floor(entityIn.posZ);
            final int k = 32;
        ...
        final int l = MathHelper.floor(entityIn.posX / 16.0);
        final int i2 = MathHelper.floor(entityIn.posY / 16.0);
        final int j2 = MathHelper.floor(entityIn.posZ / 16.0);
        if (!entityIn.addedToChunk || entityIn.chunkCoordX != l || entityIn.chunkCoordY != i2 || entityIn.chunkCoordZ != j2) {
            ...
            if (!entityIn.setPositionNonDirty() && !this.isChunkLoaded(l, j2, true)) {
    ...
    public boolean checkNoEntityCollision(final Box bb) {
        return this.checkNoEntityCollision(bb, null);
    }
    
    public boolean checkNoEntityCollision(final Box bb, @Nullable final Entity entityIn) {
    ...
    public boolean checkBlockCollision(final Box bb) {
        ...
        final int i2 = MathHelper.floor(bb.minZ);
        final int j2 = MathHelper.ceil(bb.maxZ);
        final BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain();
        for (int k2 = i; k2 < j; ++k2) {
            for (int l2 = k; l2 < l; ++l2) {
                for (int i3 = i2; i3 < j2; ++i3) {
                    final IBlockState iblockstate = this.getBlockState(blockpos$pooledmutableblockpos.setPos(k2, l2, i3));
                    if (iblockstate.getMaterial() != Material.AIR) {
                        blockpos$pooledmutableblockpos.release();
                        return true;
                    }
    ...
    public boolean containsAnyLiquid(final Box bb) {
        final int i = MathHelper.floor(bb.minX);
        final int j = MathHelper.ceil(bb.maxX);
        final int k = MathHelper.floor(bb.minY);
        final int l = MathHelper.ceil(bb.maxY);
    ...
    public boolean isFlammableWithin(final Box bb) {
        final int i = MathHelper.floor(bb.minX);
        final int j = MathHelper.ceil(bb.maxX);
        final int k = MathHelper.floor(bb.minY);
        final int l = MathHelper.ceil(bb.maxY);
        ...
        if (this.isAreaLoaded(i, k, i2, j, l, j2, true)) {
            ...
            for (int k2 = i; k2 < j; ++k2) {
                for (int l2 = k; l2 < l; ++l2) {
                    for (int i3 = i2; i3 < j2; ++i3) {
                        final Block block = this.getBlockState(blockpos$pooledmutableblockpos.setPos(k2, l2, i3)).getBlock();
    ...
    public boolean handleMaterialAcceleration(final Box bb, final Material materialIn, final Entity entityIn) {
        final int i = MathHelper.floor(bb.minX);
        final int j = MathHelper.ceil(bb.maxX);
        final int k = MathHelper.floor(bb.minY);
        final int l = MathHelper.ceil(bb.maxY);
    ...
    public boolean isMaterialInBB(final Box bb, final Material materialIn) {
        final int i = MathHelper.floor(bb.minX);
        final int j = MathHelper.ceil(bb.maxX);
        final int k = MathHelper.floor(bb.minY);
        final int l = MathHelper.ceil(bb.maxY);
        ...
        for (int k2 = i; k2 < j; ++k2) {
            for (int l2 = k; l2 < l; ++l2) {
                for (int i3 = i2; i3 < j2; ++i3) {
                    if (this.getBlockState(blockpos$pooledmutableblockpos.setPos(k2, l2, i3)).getMaterial() == materialIn) {
                        blockpos$pooledmutableblockpos.release();
                        return true;
                    }
                }
    ...
    public Explosion createExplosion(@Nullable final Entity entityIn, final double x, final double y, final double z, final float strength, final boolean isSmoking) {
    ...
    public float getBlockDensity(final Vec3d vec, final Box bb) {
    ...
    @Nullable
    private TileEntity getPendingTileEntityAt(final BlockPos p_189508_1_) {
        for (int i = 0; i < this.addedTileEntityList.size(); ++i) {
            final TileEntity tileentity = this.addedTileEntityList.get(i);
    ...
    public boolean isBlockFullCube(final BlockPos pos) {
        final Box axisalignedbb = this.getBlockState(pos).getCollisionBoundingBox(this, pos);
    ...
    public boolean canBlockFreeze(final BlockPos pos, final boolean noWaterAdj) {
        ...
        final float f = biome.getFloatTemperature(pos);
        if (f >= 0.15f) {
            return false;
        }
    ...
    private int getRawLight(final BlockPos pos, final EnumSkyBlock lightType) {
        ...
        try {
            ...
            for (int length = (values = EnumFacing.values()).length, n = 0; n < length; ++n) {
                ...
                final int k = this.getLightFor(lightType, blockpos$pooledmutableblockpos) - j;
                if (k > i) {
                    i = k;
                }
                if (i >= 14) {
        ...
        if (l > k) {
            ...
            while (i < j) {
                ...
                if (i4 == l3) {
                    ...
                    final int j4 = MathHelper.abs(i3 - i2);
                    final int k4 = MathHelper.abs(j3 - j2);
                    final int l4 = MathHelper.abs(k3 - k2);
                    if (j4 + k4 + l4 >= 17) {
                        continue;
                    }
                    final BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain();
                    ...
                    for (int length = (values = EnumFacing.values()).length, n = 0; n < length; ++n) {
                        ...
                        blockpos$pooledmutableblockpos.setPos(i5, j5, k5);
                        final int l5 = Math.max(1, this.getBlockState(blockpos$pooledmutableblockpos).getLightOpacity());
                        i4 = this.getLightFor(lightType, blockpos$pooledmutableblockpos);
    ...
    public List<Entity> getEntitiesWithinAABBExcludingEntity(@Nullable final Entity entityIn, final Box bb) {
        return this.getEntitiesInAABBexcluding(entityIn, bb, EntitySelectors.NOT_SPECTATING);
    }
    
    public List<Entity> getEntitiesInAABBexcluding(@Nullable final Entity entityIn, final Box boundingBox, @Nullable final Predicate<? super Entity> predicate) {
        final List<Entity> list = Lists.newArrayList();
        final int i = MathHelper.floor((boundingBox.minX - 2.0) / 16.0);
        final int j = MathHelper.floor((boundingBox.maxX + 2.0) / 16.0);
        final int k = MathHelper.floor((boundingBox.minZ - 2.0) / 16.0);
        final int l = MathHelper.floor((boundingBox.maxZ + 2.0) / 16.0);
    ...
    public <T extends Entity> List<T> getEntitiesWithinAABB(final Class<? extends T> classEntity, final Box bb) {
        return this.getEntitiesWithinAABB(classEntity, bb, (com.google.common.base.Predicate<? super T>)EntitySelectors.NOT_SPECTATING);
    }
    
    public <T extends Entity> List<T> getEntitiesWithinAABB(final Class<? extends T> clazz, final Box aabb, @Nullable final Predicate<? super T> filter) {
        final int i = MathHelper.floor((aabb.minX - 2.0) / 16.0);
        final int j = MathHelper.ceil((aabb.maxX + 2.0) / 16.0);
        final int k = MathHelper.floor((aabb.minZ - 2.0) / 16.0);
        final int l = MathHelper.ceil((aabb.maxZ + 2.0) / 16.0);
        final List<T> list = Lists.newArrayList();
        for (int i2 = i; i2 < j; ++i2) {
            for (int j2 = k; j2 < l; ++j2) {
                if (this.isChunkLoaded(i2, j2, true)) {
    ...
    @Nullable
    public <T extends Entity> T findNearestEntityWithinAABB(final Class<? extends T> entityType, final Box aabb, final T closestTo) {
    ...
    public boolean func_190527_a(final Block p_190527_1_, final BlockPos p_190527_2_, final boolean p_190527_3_, final EnumFacing p_190527_4_, @Nullable final Entity p_190527_5_) {
        final IBlockState iblockstate = this.getBlockState(p_190527_2_);
        final Box axisalignedbb = p_190527_3_ ? null : p_190527_1_.getDefaultState().getCollisionBoundingBox(this, p_190527_2_);
        return (axisalignedbb == Block.NULL_AABB || this.checkNoEntityCollision(axisalignedbb.offset(p_190527_2_), p_190527_5_)) && ((iblockstate.getMaterial() == Material.CIRCUITS && p_190527_1_ == Blocks.ANVIL) || (iblockstate.getMaterial().isReplaceable() && p_190527_1_.canPlaceBlockOnSide(this, p_190527_2_, p_190527_4_)));
    }
    ...
    @Nullable
    public EntityPlayer func_190525_a(final double p_190525_1_, final double p_190525_3_, final double p_190525_5_, final double p_190525_7_, final Predicate<Entity> p_190525_9_) {
        double d0 = -1.0;
        EntityPlayer entityplayer = null;
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            final EntityPlayer entityplayer2 = this.playerEntities.get(i);
            if (p_190525_9_.apply((Object)entityplayer2)) {
                final double d2 = entityplayer2.getDistanceSq(p_190525_1_, p_190525_3_, p_190525_5_);
                if ((p_190525_7_ < 0.0 || d2 < p_190525_7_ * p_190525_7_) && (d0 == -1.0 || d2 < d0)) {
                    d0 = d2;
    ...
    @Nullable
    public EntityPlayer getNearestAttackablePlayer(final double posX, final double posY, final double posZ, final double maxXZDistance, final double maxYDistance, @Nullable final Function<EntityPlayer, Double> playerToDouble, @Nullable final Predicate<EntityPlayer> p_184150_12_) {
        double d0 = -1.0;
        EntityPlayer entityplayer = null;
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            final EntityPlayer entityplayer2 = this.playerEntities.get(i);
            if (!entityplayer2.abilities.disableDamage && entityplayer2.isEntityAlive() && !entityplayer2.isSpectator() && (p_184150_12_ == null || p_184150_12_.apply((Object)entityplayer2))) {
                ...
                if (playerToDouble != null) {
                    d3 *= (double)MoreObjects.firstNonNull((Object)playerToDouble.apply((Object)entityplayer2), (Object)1.0);
                }
    ...
    public void joinEntityInSurroundings(final Entity entityIn) {
        ...
        for (int l = -2; l <= 2; ++l) {
            for (int i2 = -2; i2 <= 2; ++i2) {
                this.getChunkFromChunkCoords(i + l, j + i2);
            }
        }
    ...
    public float getRainStrength(final float delta) {
        if (WurstClient.INSTANCE.hax.noWeatherMod.isRainDisabled()) {
            return 0.0f;
        }
    ...
    public boolean isRainingAt(final BlockPos strikePosition) {
        ...
        final Biome biome = this.getBiome(strikePosition);
        return !biome.getEnableSnow() && !this.canSnowAt(strikePosition, false) && biome.canRain();
    }
    ...
    public void sendBlockBreakProgress(final int breakerId, final BlockPos pos, final int progress) {
        for (int i = 0; i < this.eventListeners.size(); ++i) {
            final IWorldEventListener iworldeventlistener = this.eventListeners.get(i);
