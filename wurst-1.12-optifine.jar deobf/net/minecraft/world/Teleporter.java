...
public class Teleporter
{
    ...
    public boolean placeInExistingPortal(final Entity entityIn, final float rotationYaw) {
                ...
                for (int j2 = -128; j2 <= 128; ++j2) {
                    ...
                    for (BlockPos blockpos3 = blockpos2.add(i2, this.worldServerInstance.getActualHeight() - 1 - blockpos2.getY(), j2); blockpos3.getY() >= 0; blockpos3 = blockpos4) {
                        ...
                        if (this.worldServerInstance.getBlockState(blockpos3).getBlock() == Blocks.PORTAL) {
                            for (blockpos4 = blockpos3.down(); this.worldServerInstance.getBlockState(blockpos4).getBlock() == Blocks.PORTAL; blockpos4 = blockpos4.down()) {
                                blockpos3 = blockpos4;
                            }
        ...
        if (d0 >= 0.0) {
            ...
            final BlockPattern.PatternHelper blockpattern$patternhelper = Blocks.PORTAL.createPatternHelper(this.worldServerInstance, blockpos);
            ...
            final double d6 = blockpattern$patternhelper.getFrontTopLeft().getY() + 1 - entityIn.getLastPortalVec().yCoord * blockpattern$patternhelper.getHeight();
            if (flag2) {
                ++d5;
            }
            if (blockpattern$patternhelper.getForwards().getAxis() == EnumFacing.Axis.X) {
                d4 = d5 + (1.0 - entityIn.getLastPortalVec().xCoord) * blockpattern$patternhelper.getWidth() * blockpattern$patternhelper.getForwards().rotateY().getAxisDirection().getOffset();
            }
            else {
                d3 = d5 + (1.0 - entityIn.getLastPortalVec().xCoord) * blockpattern$patternhelper.getWidth() * blockpattern$patternhelper.getForwards().rotateY().getAxisDirection().getOffset();
            }
            float f = 0.0f;
            float f2 = 0.0f;
            float f3 = 0.0f;
    ...
    public boolean makePortal(final Entity entityIn) {
        ...
        for (int j3 = j - 16; j3 <= j + 16; ++j3) {
            ...
            Label_0446:
                for (int j4 = this.worldServerInstance.getActualHeight() - 1; j4 >= 0; --j4) {
                    if (this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(j3, j4, l3))) {
                        while (j4 > 0 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(j3, j4 - 1, l3))) {
                            --j4;
                        }
                            ...
                            for (int j5 = 0; j5 < 3; ++j5) {
                                for (int k4 = 0; k4 < 4; ++k4) {
                                    for (int l5 = -1; l5 < 4; ++l5) {
                                        ...
                                        if (l5 < 0 && !this.worldServerInstance.getBlockState(blockpos$mutableblockpos).getMaterial().isSolid()) {
                                            continue Label_0446;
                                        }
            ...
            for (int l6 = j - 16; l6 <= j + 16; ++l6) {
                ...
                Label_0819:
                    for (int i6 = this.worldServerInstance.getActualHeight() - 1; i6 >= 0; --i6) {
                        if (this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(l6, i6, j7))) {
                            while (i6 > 0 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(l6, i6 - 1, j7))) {
                                --i6;
                            }
        ...
        if (d0 < 0.0) {
            j2 = (k7 = MathHelper.clamp(j2, 70, this.worldServerInstance.getActualHeight() - 10));
