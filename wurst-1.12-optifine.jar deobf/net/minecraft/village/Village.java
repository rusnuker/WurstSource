...
public class Village
{
        ...
        for (int k = i; k < i + blockSize.getX(); ++k) {
            for (int l = blockLocation.getY(); l < blockLocation.getY() + blockSize.getY(); ++l) {
                for (int i2 = j; i2 < j + blockSize.getZ(); ++i2) {
                    if (this.worldObj.getBlockState(new BlockPos(k, l, i2)).isNormalCube()) {
                        return false;
                    }
                }
            }
    ...
    private void updateNumIronGolems() {
        final List<EntityIronGolem> list = this.worldObj.getEntitiesWithinAABB((Class<? extends EntityIronGolem>)EntityIronGolem.class, new Box(this.center.getX() - this.villageRadius, this.center.getY() - 4, this.center.getZ() - this.villageRadius, this.center.getX() + this.villageRadius, this.center.getY() + 4, this.center.getZ() + this.villageRadius));
        this.numIronGolems = list.size();
    }
    
    private void updateNumVillagers() {
        final List<EntityVillager> list = this.worldObj.getEntitiesWithinAABB((Class<? extends EntityVillager>)EntityVillager.class, new Box(this.center.getX() - this.villageRadius, this.center.getY() - 4, this.center.getZ() - this.villageRadius, this.center.getX() + this.villageRadius, this.center.getY() + 4, this.center.getZ() + this.villageRadius));
    ...
    public void readVillageDataFromNBT(final NBTTagCompound compound) {
        ...
        for (int j = 0; j < nbttaglist2.tagCount(); ++j) {
            ...
            if (nbttagcompound2.hasKey("UUID") && this.worldObj != null && this.worldObj.getMinecraftServer() != null) {
                final PlayerProfileCache playerprofilecache = this.worldObj.getMinecraftServer().getPlayerProfileCache();
