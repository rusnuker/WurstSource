...
@Bypasses
public final class TriggerBotMod extends Hack implements UpdateListener
{
    ...
    public TriggerBotMod() {
        ...
        this.useCooldown = new CheckboxSetting("Use Attack Cooldown as Speed", true) {
            @Override
            public void update() {
                TriggerBotMod.this.speed.setDisabled(this.isChecked());
            }
        };
    ...
    public Feature[] getSeeAlso() {
        return new Feature[] { TriggerBotMod.WURST.hax.killauraLegitMod, TriggerBotMod.WURST.special.targetSpf };
    }
    
    @Override
    public void onEnable() {
        TriggerBotMod.WURST.hax.clickAuraMod.setEnabled(false);
        TriggerBotMod.WURST.hax.fightBotMod.setEnabled(false);
        TriggerBotMod.WURST.hax.killauraLegitMod.setEnabled(false);
        TriggerBotMod.WURST.hax.killauraMod.setEnabled(false);
        TriggerBotMod.WURST.hax.multiAuraMod.setEnabled(false);
        TriggerBotMod.WURST.hax.protectMod.setEnabled(false);
        TriggerBotMod.WURST.hax.tpAuraMod.setEnabled(false);
        TriggerBotMod.EVENTS.add(UpdateListener.class, this);
    }
    
    @Override
    public void onDisable() {
        TriggerBotMod.EVENTS.remove(UpdateListener.class, this);
    }
    ...
    public void onUpdate() {
        ...
        if (TriggerBotMod.MC.objectMouseOver == null || !this.isCorrectEntity(TriggerBotMod.MC.objectMouseOver.entityHit)) {
            return;
        }
        WPlayer.prepareAttack();
        WPlayer.attackEntity(TriggerBotMod.MC.objectMouseOver.entityHit);
        this.updateLastMS();
    }
    
    private boolean isCorrectEntity(final Entity entity) {
        final EntityPlayerSP player = WMinecraft.getPlayer();
        final double rangeSq = Math.pow(this.range.getValue(), 2.0);
        Stream<Entity> stream = Stream.of(entity).filter(e -> e != null && !e.isDead).filter(e -> (e instanceof EntityLivingBase && ((EntityLivingBase)e).getHealth() > 0.0f) || e instanceof EntityEnderCrystal).filter(e -> entityPlayerSP.getDistanceSqToEntity(e) <= n).filter(e -> e != entityPlayerSP2).filter(e -> !(e instanceof EntityFakePlayer)).filter(e -> EntityUtils.isCorrectTeam(e)).filter(e -> !TriggerBotMod.WURST.getFriends().contains(e.getName()));
